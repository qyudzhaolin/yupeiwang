    ...
    public function save()
    {
        ...
        if($node_id > 0){
            ...
            if($res){
                ...
                $value = app::get('content')->model('article_nodes')->select()->where('node_id = ?', $node_id)->instance()->fetch_row();
                kernel::single('content_article_node')->store_node_kvstore($node_id, $value);
            ...
            if($res){
                ...
                $value = app::get('content')->model('article_nodes')->select()->where('node_id = ?', $post['node_id'])->instance()->fetch_row();
                kernel::single('content_article_node')->store_node_kvstore($post['node_id'], $value);
