...
class ectools_ctl_payment_cfgs extends desktop_controller{
    ...
    public function _views(){
        ...
        $show_menu = array(
            1=>array('label'=>app::get('ectools')->_('全部'),'optional'=>false,'filter'=>array('is_frontend' => false)),
            2=>array('label'=>app::get('ectools')->_('标准版'),'optional'=>false,'filter'=>array('platform'=>'ispc', 'is_frontend' => false)),
            3=>array('label'=>app::get('ectools')->_('触屏版'),'optional'=>false,'filter'=>array('platform'=>'iswap', 'is_frontend' => false))
        );
        ...
        foreach($show_menu as $var_k=>&$var_v){
            if ($var_k == intval($_GET['view'])){
                $var_v["newcount"] = true;
                $var_v["addon"] = count($o->getList('*',$var_v["filter"]));
            }
        }
        unset($var_v);
