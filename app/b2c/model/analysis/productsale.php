...
class b2c_mdl_analysis_productsale extends dbeav_model{
    ...
    public function getlist($cols='*', $filter=array(), $offset=0, $limit=-1, $orderType=null){
        ...
        foreach($rows as $key=>$val){
            ...
            $sql = 'SELECT sum(number) as refund_num FROM '.
                ...
                kernel::database()->prefix.'b2c_reship as R ON I.reship_id=R.reship_id '.$this->get_reship_filter($filter,$val["rownum"]);
            $row = $this->db->select($sql);
    ...
    //获取退款单表的filter
    private function get_reship_filter($filter,$goods_id){
        $where = array("R.status='succ'");
        //获取product_id
        $product_info = app::get('b2c')->model('products')->getList('product_id', array("goods_id"=>$goods_id));
        if(!empty($product_info)){
            $arr_product_ids = array();
            foreach ($product_info as $var_p_i){
                $arr_product_ids[] = $var_p_i["product_id"];
            }
            $where[] = "I.product_id in(".implode(",",$arr_product_ids).")";
        }
        if(isset($filter['time_from']) && $filter['time_from']){
            $where[] = ' R.t_begin >='.strtotime($filter['time_from']);
        }
        if(isset($filter['time_to']) && $filter['time_to']){
            $where[] = ' R.t_begin <'.(strtotime($filter['time_to'])+86400);
        }
