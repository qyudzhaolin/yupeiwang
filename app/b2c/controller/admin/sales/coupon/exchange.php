...
class b2c_ctl_admin_sales_coupon_exchange extends desktop_controller{
    ...
    function index() {
        $this->finder('b2c_mdl_coupons', array(
                ...
                'actions'=>array(
                                array('label'=>app::get('b2c')->_('添加兑换规则'),'href'=>'index.php?app=b2c&ctl=admin_sales_coupon_exchange&act=add','target'=>'dialog::{title:\'添加兑换规则\',width:460,height:195}'),
                                array('label'=>app::get('b2c')->_('删除'), 'submit'=>'index.php?app=b2c&ctl=admin_sales_coupon_exchange&act=delete'),
                            ),'finder_cols'=>'column_edit,cpns_name,cpns_point,cpns_max_num','finder_aliasname'=>'exchange',
    ...
    public function save() {
        ...
        $arr['cpns_max_num'] = $_POST['cpns_max_num'];
        $flag = false;
        if($arr['cpns_id']) {
