        ...
        if($gask_type){
            ...
            foreach($show_menu as $var_k=>&$var_v){
                if ($var_k == intval($_GET['view'])){
                    $var_v["newcount"] = true;
                    $var_v["addon"] = $member_comments->count($var_v["filter"]);
                }
            }
            unset($var_v);
 ...
 #设置显示
         ...
         if($member_comments->save($sdf)){
            if($sdf['display']=='true' && $sdf['object_type'] == 'discuss' && $sdf['author_id']){
                ...
                if($_is_add_point){
                    ...
                    kernel::single('pointprofessional_point_common')->point_change_action($sdf['author_id'],$_is_add_point,$msg,'comment_discuss',2,$sdf['type_id'],$sdf['author_id'],'comment');
                }
            }
    ...
    function delete_message($comment_id){
      ...
      if($member_comment->delete(array('comment_id' => $aComId))){
		...
		$data['uname'] = $aData['author'];
		$data['goodsurl'] = app::get('site')->router()->gen_url(array('app'=>'b2c','ctl'=>'site_product','act'=>'index','arg0'=>$aData['goods_id']));
		$data['goods_name'] = $aData['goods_name'];
