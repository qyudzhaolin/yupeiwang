 ...
 * dev@shopex.cn
 */
class b2c_apiv_apis_response_order_refund
{
    /**
     * app object
     */
     ...
     * @return array
     */
    function refuse($sdf, $thisObj){
        $mdl_b2c_orders = app::get('b2c')->model('orders');
        $mdl_b2c_refund_apply = app::get('b2c')->model('refund_apply');
//         $afterinfo = app::get('aftersales')->model('return_product');

        //判断判断是否有相应的订单数据
        $order_info = $mdl_b2c_orders->dump(array('order_id'=>$sdf['tid']));
        //这里判断本地退款申请表中有相应的数据
        $rs_b2c_refund_apply = $mdl_b2c_refund_apply->dump(array("order_id"=>$sdf["tid"],"status"=>"0"));
        if (!empty($order_info) && !empty($rs_b2c_refund_apply)){
            //更新status为拒绝1 并 记录日志
            $mdl_b2c_refund_apply->update_refund_apply($rs_b2c_refund_apply["refund_apply_bn"],"1","拒绝退款申请");
            if ($order_info['ship_status'] == 0){
                ...
                $mdl_b2c_orders->update($update_info, array('order_id' => $sdf['tid']));
                $message = "售前";
            }else{
                //售后
//                 $afterinfo->update(['status' => '5'], ['order_id' => $sdf['tid']]);
                $message = "售后";
            }
            $message.= "退款申请拒绝成功!";
            return array('status'=>'true','message'=>$message);
        }else{
            return array('status'=>'false','message'=>app::get('b2c')->_('数据缺失操作失败!'));
        }
    }
