...
class b2c_order_refund extends b2c_api_rpc_request
{
    ...
    public function order_refund_finish(&$sdf, $status='succ', $from='Back', &$msg='')
    {
        ...
        $rs_refund_apply = $mdl_b2c_refund_apply->dump(array("order_id"=>$sdf['order_id'],"status"=>"0"));
        if (!empty($rs_refund_apply)){
            $result_refund = $mdl_b2c_refund_apply->update_refund_apply($rs_refund_apply["refund_apply_bn"],"2","退款申请的退款操作");
            if (!$result_refund){
                $msg = app::get('b2c')->_('退款申请的退款操作失败！');
                return false;
            }
        }
		...
		if ($is_saveOdr && $sdf_order['member_id'])
        {
			...
			foreach ((array)$arr_return_score as $arr_is_returned){
             ...
             // 退还订单消费积分
            if ($sdf['return_score'] > ($sdf_order['score_g'] - $is_returned_score)){
            ...
            $obj_point_common->point_change_action($sdf_order['member_id'],(0 - intval($sdf['return_score'])),$msg,'order_refund_use',1,$sdf['order_id'],$this->op_id,'refund');
            //zengxinwen
            if(intval($sdf['score_u'])){
                $obj_point_common->point_change_action($sdf_order['member_id'],intval($sdf['score_u']),$msg,'operator_adjust',3,$sdf['order_id'],$this->op_id,'refund');
            }
            //end
