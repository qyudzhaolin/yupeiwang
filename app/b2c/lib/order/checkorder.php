...
class b2c_order_checkorder{
    ...
    public function check_order_delivery($order_id,$sdf_post=array(),&$msg)
    {
        ...
        foreach ($order_items as $key=>$dinfo)
        {
            ...
            //超卖订单库存不足判断
            if($sdf_order["is_oversold"] == "true" && $product[0]['freez'] > $product[0]['store']){
                $msg = app::get('b2c')->_('超卖订单! 商品：').$dinfo['name'].app::get('b2c')->_("货品货号为‘".$dinfo['bn']."’的库存不足");
                return false;
            }
    ...
    //售前退款检查
    public function check_order_refund_apply($order_id,$sdf_post,&$msg){
        $order = $this->app->model('orders');
        $sdf_order = $order->dump($order_id,'*');
        if($sdf_post['money']){//退款弹窗此项不可编辑
            if($sdf_post['money'] == $sdf_order['payed'] && $sdf_order['payed']>0){
                return true;
            }else{
                $msg = "退款失败:退款金额和已支付的金额不相等";
                return false;
            }
        }
        $msg = "退款失败:退款金额不符合要求";
