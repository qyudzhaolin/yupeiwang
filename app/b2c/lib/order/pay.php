        ...
        if ($sdf_order)
        {
           ...
           $errorMsg[] = $error_Msg; 
            ...
            if (isset($sdf_order['member_id']) && $sdf_order['member_id'] && $arrOrder['payed'] == $sdf_order['cur_amount'] && $pay_status==1)
            {
                ...
                /** end **/
                if ($stage)
                    $is_change_point = $obj_reducte_point->change_point($sdf_order['member_id'], 0 - intval($sdf_order['score_u']), $msg, 'order_pay_use', 1, $rel_id, $operator);
                ...
                if ($stage)
                    $obj_add_point->change_point($sdf_order['member_id'], intval($sdf_order['score_g']), $msg, 'order_pay_get', 2, $rel_id, $operator);
            ...
            if ($arrOrder['payed'] == $sdf_order['cur_amount'])
            {
                ...
                if ($store_mark == '2')
                {
                    ...
                    //订单打超卖标
                    if($is_oversold && $sdf['pay_status'] != "FAILED"){
                        $update_is_oversold = array("order_id"=>$rel_id,"is_oversold"=>$is_oversold);
                        $obj_orders->save($update_is_oversold);
                    }
                }
