...
class b2c_order_full{
    ...
    public function get($order_id)
    {
        ...
        if($sdf['status'] == 'active'){
            $arr_data['status'] = 'TRADE_ACTIVE';
        }elseif($sdf['status'] == 'finish'){
		...
		if ($sdf['addon'])
		{
		    $arr_order_addon = unserialize($sdf['addon']);
			if ($arr_order_addon['order_chgpointmoney'] > 0)
			{
				$arr_data['goods_discount_fee'] = $this->objMath->number_plus(array($arr_data['goods_discount_fee'], $arr_order_addon['order_chgpointmoney']));
				$arr_promotion_details[] = array(
					'promotion_name'=>app::get('b2c')->_('订单积分折扣优惠'),
					'promotion_fee'=>$arr_order_addon['order_chgpointmoney'],
                    );
			}
		}
