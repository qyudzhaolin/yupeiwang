...
class b2c_finder_member_gask{
    ...
    function detail_basic($comment_id){
        ...
        if($gask_data['type_id']){
            $goods_point = $app->model('comment_goods_point');
        ...
        if($goods_data){
            if(!$gask_data['product_id']){
                $productData = app::get('b2c')->model('products')->getList('product_id',array('goods_id'=>$gask_data['type_id'],'is_default'=>'true'));
                $product_id = $productData[0]['product_id'];
            }else{
                $product_id = $gask_data['product_id'];
            }
            $render->pagedata['url'] = app::get('site')->router()->gen_url(array('app'=>'b2c','ctl'=>'site_product','full'=>1,'act'=>'index','arg'=>$product_id));
