...
class aftersales_mdl_return_product extends dbeav_model{
    ...
    /*
     * 获取退换货的顾客填写的物流信息
     * $customer_logistics 物流信息json_encode内容
     */
    public function customer_logistics_text($customer_logistics){
        //获取顾客前台填写的物流信息
        $arr_customer_logistics = json_decode($customer_logistics);
        if(!empty($arr_customer_logistics)){
            $customer_logistics_text = "";
            foreach($arr_customer_logistics as $key_c_l => $var_c_l){
                $customer_logistics_text.=$key_c_l."：".$var_c_l."&nbsp;&nbsp;&nbsp;";
            }
            return $customer_logistics_text;
        }else{
            return "";
        }
    }
