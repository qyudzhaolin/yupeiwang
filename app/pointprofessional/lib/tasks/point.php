 ...
 */
class pointprofessional_tasks_point extends base_task_abstract implements base_interface_task{
	...
	public function add_member_point_task()
	{
		...
		$sql = "SELECT * FROM " . $obj_member_point_task->table_name(1) . " WHERE task_type = '1' AND addtime != enddate AND enddate <= " . $today_time . " AND enddate > " . $yesterday_time . ' AND status = "0"';
		...
		if ($rows && is_array($rows))
		{
			...
			foreach ($rows as $arr_task)
			{
                ...
                //更新会员积分
				$total_point = $member_point->get_total_count($arr_task['member_id']);
				$obj_member->update(array('point'=>$total_point), array('member_id'=>$arr_task['member_id']));
				$total_point_cumulation = $member_point->get_total_cumulation($arr_task['member_id']);
				$obj_member->update(array('cumulation_point'=>$total_point_cumulation), array('member_id'=>$arr_task['member_id']));
	...
	public function generate_obtained_point()
	{
		...
		if ($rows)
		{
			foreach ($rows as $arr_row)
			{
				...
				$obj_member->update(array('point'=>$total_point), array('member_id'=>$arr_row['member_id']));
				$total_point_cumulation = $obj_member_point->get_total_cumulation($arr_row['member_id']);
				$obj_member->update(array('cumulation_point'=>$total_point_cumulation), array('member_id'=>$arr_row['member_id']));
			}
		}	
	}
}
