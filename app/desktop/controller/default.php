...
//      $this->pagedata['certificate_url'] = "http://key-service.shopex.cn/index.php?sess_id=".urlencode($this->pagedata['session_id'])."&certi_id=".urlencode($this->Certi)."&version=ecstore&confirmkey=".urlencode($confirmkey)."&_key_=do";
        $this->pagedata['certificate_url'] = SHOPEX_LICENSE_ACTIVE_SEARCH_URL. "?product=".kernel::single('syssupport_auth')->getProduct() . '&code=' . urlencode(kernel::single('syssupport_auth_code')->encode(kernel::single('syssupport_auth_code')->getParams(), 3600*24*7));
    ...
    {
        $gifUrl = kernel::single('base_license_active')->getGif();
        $gifUrl = $gifUrl ? : 'https://service.ec-os.net/static/images/free.gif';
        echo $gifUrl;exit;
    }
