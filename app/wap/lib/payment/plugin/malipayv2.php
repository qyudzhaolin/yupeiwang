<?php
final class wap_payment_plugin_malipayv2 extends ectools_payment_app implements ectools_interface_payment_app {

    /**
     * @var string 支付方式名称
     */
    public $name = '手机支付宝2.0';
    /**
     * @var string 支付方式接口名称
     */
    public $app_name = '手机支付宝v2';
     /**
     * @var string 支付方式key
     */
    public $app_key = 'malipayv2';
    /**
     * @var string 中心化统一的key
     */
    public $app_rpc_key = 'malipayv2';
    /**
     * @var string 统一显示的名称
     */
    public $display_name = '手机支付宝v2';
    /**
     * @var string 货币名称
     */
    public $curname = 'CNY';
     ...
     * @支付宝固定参数
     */
    public $charset  = 'utf-8';//数编码字符集
    public $sign_type = 'RSA';//签名方式
    public $alipay_gateway_new = 'https://openapi.alipay.com/gateway.do?';//支付宝网关地址
    ...
    public function __construct($app){
        ...
        $this->notify_url = kernel::openapi_url('openapi.ectools_payment/parse/wap/wap_payment_plugin_malipayv2', 'callback');
        ...
        $this->callback_url = kernel::openapi_url('openapi.ectools_payment/parse/wap/wap_payment_plugin_malipayv2', 'callback');
    ...
    public function setting(){
        return array(
            ...
            'app_id'=>array(
                'title'=>app::get('wap')->_('APP_ID'),
                'type'=>'string',
                'validate_type' => 'required',
            ),
            'product_code'=>array(
                'title'=>app::get('wap')->_('销售产品码'),
                'type'=>'string',
                'validate_type' => 'required',
            ),
            'seller_id'=>array(
...
<a target="_blank" href="https://www.alipay.com/static/utoj/utojindex.htm">'.app::get('wap')->_('如何使用支付宝支付？').'</a>';
    ...
    {
        $this->app_id = trim($this->getConf('app_id', __CLASS__));
        $this->seller_id = trim($this->getConf('seller_id', __CLASS__));
        $this->product_code = trim($this->getConf('product_code', __CLASS__));
        $this->sdk_verify_cert_dir = DATA_DIR . '/cert/payment_plugin_malipayv2/';
        ...
        $parameter = array(
            "partner" => trim($this->app_id),
            "method" => "alipay.trade.wap.pay",
            "return_url"    => $this->callback_url,
            "charset" => trim(strtolower($this->charset)),
            "timestamp" => date("Y-m-d H:i:s",time()),
            "version" => "1.0",
            "notify_url"    => $this->notify_url,
            "biz_content" => json_encode($application_params),
        );
        $parameter = array_merge($parameter,$application_params);
    ...
    public function buildRequestForm($para_temp, $method, $button_name) {
        ...
        $sHtml = $sHtml."<form id='alipaysubmit' name='alipaysubmit' action='".$this->alipay_gateway_new."charset=".trim(strtolower($this->charset))."' method='".$method."'>";
    ...
    public function callback(&$recv)
    {
        ...
        if($verify_result){
            ...
            $ret['account'] = $this->app_id;
            $ret['bank'] = app::get('wap')->_('手机支付宝2.0');
