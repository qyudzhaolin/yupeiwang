        ...
        if($params['module']){
            if(class_exists($params['module']) && ($passport_module = kernel::single($params['module']))){
                if($passport_module instanceof pam_interface_passport){
                    ...
                    $is_back = ($params['type'] == 'shopadmin') ? true : false;
                    if($is_back){
                        $sessionLib->is_back_sess_key();
                    }

                    if($_COOKIE['autologin'] > 0 && $is_back){
                        $sessionLib->set_cookie_expires($_COOKIE['autologin']);
                        //如果自动登录，设置cookie过期时间，单位：分
                    }
                    if($_COOKIE['S']['SIGN']['AUTO'] > 0 && !$is_back){
                        $minutes = 14*24*60;
                        $sessionLib->set_cookie_expires($minutes);
                    }
