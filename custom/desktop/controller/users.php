...
class desktop_ctl_users extends desktop_controller{
    ...
    function addnew(){
        ...
        if($_POST){
            ...
            if($users->validate($_POST,$msg)){
                ...
                //所属网点
                $_POST['networks'] = implode(',', $_POST['networks']);
                if($users->save($_POST)){
    ...
    public function getNetworks(){
        ...
        $networks=$mdl_networks->getList('networks_id,networks_name',['disabled'=>'false']);
        return $networks; 
    }
        ...
        if($_POST){
            ...
            elseif($_POST['super'] == 0 && $_POST['role']){
                if (empty($_POST['networks'])) {
                    $this->end(false, app::get('desktop')->_('所属网点必须选择'));
                }
                //所属网点
                $_POST['networks'] = implode(',', $_POST['networks']);
    ...
    */
    ...
    function edit($param_id){
        ...
        //获取服务网点
        $user_networks = explode(',', $sdf_users['networks']);
        $networks= $this->getNetworks();
        foreach ($networks as $k => $net) {
            if (in_array($net['networks_id'], (array)$user_networks)) {
                $networks[$k]['checked'] = 'true';
            }
        }
        $this->pagedata['networks']= $networks; 
