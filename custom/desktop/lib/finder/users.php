...
class desktop_finder_users{
    ...
    var $column_networks = '所属网点';
    function column_networks($row){
        $res = '';
        if (!empty($row['networks'])) {
            $obj_networks = app::get('b2c')->model('networks');
            $networks = $obj_networks->getListFormat('networks_id,networks_name',['disabled'=>'false']);
            $new_networks = [];//新的文字形式(华南,华北)
            $user_networks = explode(',', $row['networks']);
            foreach ($user_networks as $k => $user_network) {
                if (isset($networks[$user_network])) {
                    $new_networks[] = $networks[$user_network];
                }
            }
            $res = implode(',', $new_networks);
        }
        return $res;
    }
}
