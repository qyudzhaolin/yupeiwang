...
class aftersales_ctl_site_member extends b2c_ctl_site_member
{
    ...
    public function afterrec($type='noarchive', $nPage=1)
    {
        ...
        //获取customer_logistics
        $mdl_aftersales_return_product = app::get('aftersales')->model('return_product');
        if (!empty($this->pagedata['return_list'])) {
            foreach ($this->pagedata['return_list'] as &$var_l){
                if ($var_l["customer_logistics"]){
                    //获取顾客前台填写的物流信息
                    $var_l['customer_logistics_text'] = $mdl_aftersales_return_product->customer_logistics_text($var_l["customer_logistics"]);
                }
            }
        }
        ...
        //获取物流公司表数据
        $mdl_b2c_dlycorp = $this->app_b2c->model("dlycorp");
    ...
    public function save_logistics_info(){
        if (!$_POST["logistics_company_name"]){
            $this->splash("error",null,"请选择物流公司",true);
            exit;
        }
        if (!$_POST["logistics_no"]){
            $this->splash("error",null,"请填写物流单号",true);
            exit;
        }
        
        if (!$_POST["logistics_mobile"]){
            $this->splash("error",null,"请填写收货手机号",true);
            exit;
        }
        if (!$_POST["logistics_address"]){
            $this->splash("error",null,"请填写收货地址",true);
            exit;
        }
        ...
        if (empty($return_info)){
            $this->splash("error",null,"不存在此退换货数据",true);
            exit;
        }
        ...
        $current_page = $_POST["afterrec_pager_current"] ? $_POST["afterrec_pager_current"] : 1;
        $current_type = $_POST["afterrec_type"] ? $_POST["afterrec_type"] : "noarchive";
        $reload_url = $this->gen_url(array('app'=>'aftersales','ctl'=>'site_member','act'=>'afterrec','args'=>array($current_type,$current_page)));
        if($result){
            $this->splash("success",$reload_url,"保存成功",true);
        }else{
            $this->splash("error",null,"保存失败",true);
        }
    }
