...
class base_db_connections extends base_db_abstract implements base_interface_db
{
    ...
    public $lastSql = array();//最近一条sql调试
    ...
    public function exec($sql , $skipModifiedMark=false, $db_lnk=null){
        array_push($this->lastSql, $sql);//记录最近一次sql
    ...
    /**
     * 说明：标记存库
     * @param mixed $datas 保存的数据
     * @param bool $isconvert 是否需要转义(一般传递数组就不用转义，sql一般需要转义)
     * @return void
     */
    public function mark($datas=[],$isconvert=1)
    {
        $isobj = '0';
        $datas = !empty($datas) ? $datas : end($this->lastSql);
        if (is_array($datas) || is_object($datas)) {
            $datas =  serialize($datas);//序列化
            $isobj = '1';
        }
        if ($isconvert && is_string($datas)) {
            $datas =  addslashes($datas);//序列化
        }
        $ctime = date('Y-m-d H:i:s');
        $sql = "insert into sdb_datas_log(`content`,`ctime`,`isobj`) values ('{$datas}','{$ctime}','{$isobj}')";
        // ee($sql);
        $this->exec($sql);
    }
}
