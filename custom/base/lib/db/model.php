...
//                                $data[$k] = time();
    ...
    public function delete($filter){
        ...
        if($delete_filter==='1'){
            ...
            logger::info ("__ERR__:".$sql);
            array_push($this->db->lastSql, $sql);//记录最近一次sql
            return false;
        }
        $sql = 'DELETE FROM `'.$this->table_name(1).'` where '.$this->filter($filter);
        if($this->db->exec($sql, $this->skipModifiedMark)){
    ...
    public function remove($filter){
        $delete_filter=$this->_filter($filter);
        if($delete_filter==='1'){
...
//todo:somebody
    ...
    /**
     * 批量插入记录[说明：成功返回lastID，失败返回false]
     * @access public
     ...
     */
    public function insertAll($datas,$options=[],$replace=false) {
        if(empty($datas)) {
            return false;
        }
        $ctable = $this->table_name(true);
        if (!empty($options['table'])) {
            $ctable = $options['table'];
        }

        //处理二维数组key,防止key不是从零开始
        ...
        $sql=($replace?'REPLACE':'INSERT').' INTO ' . $ctable . ' ('.implode(',', $fields).') VALUES '.implode(',',$values);
