...
class b2c_mdl_members extends dbeav_model{
    ...
    function count($filter=null){
        $cols = 'substring_index(area, ":",-1) as region_id';
     ...
     */
    public function getList($cols='*', $filter=array(), $offset=0, $limit=-1, $orderType=null, $dbPoint=false)
    {
        $cols .= ',substring_index(area, ":",-1) as region_id';//用于判断是否为预售订单
	    $arr_member = parent::getList($cols, $filter, $offset, $limit, $orderType);
        // ee(sql());
	    //后端管理会员列表页面显示、拿原表的point字段 不走要去打积分查询的接口
	    if ($filter["bg_mg_member_list_page"]){
	        unset($filter["bg_mg_member_list_page"]);
	    }else if($dbPoint){//直接拿db字段的point值
	    }
// 	    else if($cols=='*' || strstr($cols, 'point')){
// 	        //默认有查包括point字段的走get_total_count统一方法（里面涉及积分查询接口）
