...
class b2c_mdl_orders extends dbeav_model{
    ...
    function count($filter=null){
        $cols = 'substring_index(ship_area, ":",-1) as region_id';
        $row = $this->db->select('SELECT '. $cols .' FROM `'.$this->table_name(1).'` WHERE '.$this->_filter($filter));
        return count($row);
    }
    ...
    public function fetchByMember($member_id, $nPage=1, $order_status=array(),$limit=10)
    {
        ...
        else
        {
            ...
            if (isset($order_status['received_status']))
                $filter['received_status'] = $order_status['received_status'];
            if (isset($order_status['promotion_type|noequal']))
                $filter['promotion_type|noequal'] = $order_status['promotion_type|noequal'];
        }
    ...
    {
        $cols .= ',promotion_type,substring_index(ship_area, ":",-1) as region_id';//用于判断是否为预售订单
        ...
        foreach ($arr_list as $key => $value) {
            if($value['payment']){
                if($value['payment']=='period'){
                  $arr_list[$key]['payment']='账期支付';
                }
            }
            ...
            if($value['promotion_type']=='prepare'){
                ...
                $sql = "SELECT group_concat(DISTINCT p.pay_app_id) as payments FROM `sdb_ectools_payments` p LEFT JOIN sdb_ectools_order_bills b ON b.bill_id = p.payment_id
                    WHERE b.rel_id = '{$order_id}' AND `status` = 'succ' group by b.rel_id";
                $paymentRows = $this->db->select($sql);
                if ($paymentRows) {
                    $payments = explode(',', $paymentRows[0]['payments']);
                    if(count($payments) > 1){
                        $arr_list[$key]['payment'] = '混合支付';
                    }
                }
            }
        }
    ...
    public function searchOptions(){
        ...
        //预售订单新增预售活动号搜索
        if ($_GET['view'] == '15') {
            //$columns['active_num'] = app::get('b2c')->_('预售活动号');
        }
    ...
    */
    ...
    /**
     * 说明：当购买宇培管库商品时候检测是否合法购买限制
     * 逻辑需求：只能买一个供应商的货，只能一个仓库出库，且管库与不管库不能合并购物
     * 程序逻辑：1.默认通过，2.no条件[如果宇培管库与非管库混合]，3.no条件[只有宇培管库(含有供应商和仓库不同)]
     * @params array $acart 购物车数据
     * @return array 返回是否合法
     */
    function checkYPstoreWhenBuy(&$acart){
        $res = ['error'=>0];
        if (empty($acart) || empty($acart['object']['goods'])) {
            return $res;
        }

        $goods = $acart['object']['goods'];//商品list
        if (empty($goods)) {
            return $res;
        }
        ...
        $isStoreArr = [];
        foreach ($goods as $go) {
            $product = $go['obj_items']['products'][0];
            $isStoreArr[] = $product['is_yp_store'];
        }
        $isStoreArr = array_unique($isStoreArr);

        if (count($isStoreArr) > 1) {
            return ['error'=>1,'message'=>"宇培管库和非管库商品不能混合购买！"];
        }

        if (count($isStoreArr) == 1 && $isStoreArr[0] == '1') {
            //检测1：只有宇培管库商品
            $storehouse_id_arr = $owner_code_arr = [];
            foreach ($goods as $good) {
                $product = $good['obj_items']['products'][0];
                $storehouse_id_arr[] = $product['storehouse_id'];
                $owner_code_arr[] = $product['owner_code'];

                $productName = $product['name'];
                if (count(array_unique($storehouse_id_arr)) > 1) {
                    return ['error'=>1,'message'=>"{$productName}与其他商品必须在同一个仓库！"];
                }elseif (count(array_unique($owner_code_arr)) > 1) {
                    return ['error'=>1,'message'=>"{$productName}与其他商品必须为同一个供应商！"];
                }
