...
class b2c_mdl_cart_objects extends dbeav_model{
    ...
    public function save_contract( &$data,$mustUpdate = null ,$mustInsert = false){
        $arr_member_info = $this->get_member_info();
        $data['member_ident'] = $this->get_md5_ident( $arr_member_info['member_id'] );
        $data['member_id'] = ( $arr_member_info['member_id'] ? $arr_member_info['member_id'] : -1 );
        if( empty( $data['member_id'] ) ) return false;
        ...
        //删除之前出库的合约商品
        $wheres['member_id'] = $data['member_id'];
        $wheres['buy_type'] = 'contract';
        //注意这里必须用parent::因为此类有覆盖
        if (parent::count($wheres)) {
            $this->remove($wheres);
        }
        ...
        $sdata = [];
        $ctime = time();
        $contractGoods = $data['goods'];
        foreach ($contractGoods as $g) {
            $tmp                 = [];
            $tmp['obj_ident']    = $g['obj_ident'];
            $tmp['member_ident'] = $data['member_ident'];
            $tmp['obj_type']     = $data['obj_type'];
            $tmp['member_id']    = $data['member_id'];
            $tmp['quantity']     = $g['quantity'];
            $tmp['time']         = $ctime;
            $tmp['buy_type']     = 'contract';//标记合约
            $tmp['params']       = [
                'goods_id'       => $g['goods_id'],
            ...
            $tmp['params'] = serialize($tmp['params']);
            $sdata[] = $tmp;
        }
        $result = $this->insertAll($sdata,['table'=>'sdb_b2c_cart_objects']);
        // ee(sql(0));
        // ee($sdata);
        return $result;
    }
    ...
    public function parent_count( $filter=array() ) {
        return parent::count( $filter );
    }
    ...
    private function parent_getlist( $cols, $tmp, $offset, $limit, $orderType ) {
        if($tmp['is_fastbuy'] == 'true' || $this->is_fastbuy==true){
            ...
            // mark($tmp);
            // ee($_SESSION['b2c_cart_objects_fastbuy'][$tmp['member_ident']]);
        ...
        if( $tmp['member_id']=='-1' ) {
            ...
            if( $this->use_kv() && !$this->is_contract()) { //使用kv
                $this->kv_instance()->fetch( $this->kv_prefix,$arr );
                $arr = $this->filter_getlist( $arr,$tmp );
                return $arr ? $arr : array();
            } else {  //使用database
                //合约
                if ($this->is_contract()) {
                   $tmp['buy_type'] = 'contract';
                }else{
                    $tmp['buy_type'] = '';
                }
                $res = parent::getList( $cols, $tmp, $offset, $limit, $orderType );
                // ee(sql(),0);
                return $res;
            }
        }
