...
class b2c_widgets_goods extends b2c_widgets_public {
    ...
    protected $_outData = array(
        ...
        'areaId'            => 'provenance_id',        //原产地
        'catId'             => 'cat_id',        //分类id
        'sales_price'       => 'sales_price',        //销售标准价
        'unit'              => 'unit',        //单位
    );
    ...
    public function getGoodsList($filter,$platform='site'){
        ...
        // $this->app->model('shiparea')->makeGoodsfilterByShiparea($_filter,false);//配送区域过滤
        // ee($_filter);
        ...
        if($gids){
            $productData = $objProduct->getList('goods_id,product_id,marketable,price,mktprice',array('goods_id'=>$gids,'is_default'=>'true'));
		...
		if(!empty($val['mktprice'])){
                    $result['goodsRows'][$val['goods_id']]['goodsMarketPrice']=$val['mktprice'];
                 }
