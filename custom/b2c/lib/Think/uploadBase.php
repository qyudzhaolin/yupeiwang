...
class uploadBase {
    ...
    private function setDriver($driver = null, $config = null){
        ...
        $config = $config ? : ($this->driverConfig ? : getConfig('UPLOAD_TYPE_CONFIG'));
        $className = ucfirst(strtolower($driver));
        // $class = strpos($driver,'\\')? $driver : 'Think\\Upload\\Driver\\'.ucfirst(strtolower($driver));
        require_once getConfig('Think') . "Upload/Driver/{$className}.php";
        $this->uploader = new $className($config);
        if(!$this->uploader){
            E("不存在上传驱动：{$name}");
        }
    }
