...
class b2c_order_create extends b2c_api_rpc_request
{
    ...
    public function generate(&$sdf, $member_indent='', &$msg='',$aCart=array())
    {
        ...
        if ($odr_create_service)
        {
            ...
            {
                //progetcoupon_order_beforecreate-----couponlog_order_beforecreate-----
                $odr_ser->generate($order_data,$sdf,$cart_objects);
            }
        }
        // ee($order_data);
        //把普通商品的数据改为适应合约的，例如总价、合约商品单价、多件合约商品价格等
        // if (defined('BUYTYPE') && BUYTYPE == 'contract') {
        //     foreach ($order_data['order_objects'] as $key => $value) {
        //         $order_data['order_objects'][$key]
        //     }
        // }
        return $order_data;
    }
    ...
    private function _chgdata(&$sdf, &$order_data, $member_indent='', &$msg='',$aCart=array())
    {
        ...
        $order_data = array(
            ...
            'tax_code' => $sdf['payment']['tax_code'],
            'tax_bank' => $sdf['payment']['tax_bank'],
            'bank_account' => $sdf['payment']['bank_account'],
            'tax_addr' => $sdf['payment']['tax_addr'],
            'tax_remark' => $sdf['payment']['tax_remark'],
        );
       if( $sdf['payment']['pay_app_id']==-1){
           $order_data['payment_offline']='-1';
        ...
        //保证金订单类型
        if($sdf['order_type']=='bail'){
           $order_data['promotion_type']="bail";
           $order_data['total_amount']=$sdf['order_money'];
           $order_data['cur_amount']=$sdf['order_money'];
        }
                                ...
                                $order_data['order_objects'][$index] = array(
                                    ...
                                    'order_items' => array(
                                        array(
                                            ...
                                            //宇培管库相关
                                            'storehouse_id'=>$arrObjItems['obj_items']['products'][0]['storehouse_id'],
                                            'owner_code'=>$arrObjItems['obj_items']['products'][0]['owner_code'],
                                            'sku_code'=>$arrObjItems['obj_items']['products'][0]['sku_code'],
                                            'is_yp_store'=>$arrObjItems['obj_items']['products'][0]['is_yp_store'],
                                            'unitEn'=>$arrObjItems['obj_items']['products'][0]['unitEn'],
                                            'unit'=>$arrObjItems['obj_items']['products'][0]['unit'],
    ...
    public function save(&$sdf, &$msg='')
    {
        ...
        {   //预售判断减库存
            ...
            if($pre)
            {
                ...
                $pre->order_notify($product_id,$sdf);
            }
            return true;
        }
    }
