...
class b2c_order_delivery extends b2c_order_operation
{
    ...
    public function generate($sdf, &$controller=null, &$msg='')
    {
        ...
        */
        ...
        //API//推到WMS系统,生成出库单
        $obj_order_items = $controller->app->model('order_items');
        $orderItems = $obj_order_items->getList('*',array('order_id'=>$sdf['order_id']));
        if (!empty($orderItems) && $orderItems[0]['is_yp_store'] == '1') {
            $res = $controller->createSoOder($sdf, $orderItems);//开始生成wms出库单
            if ($res['error']) {
                $msg = $res['message'];
                return false;
            }else{
                //成功后在订单表字段soOrderNum写入WMS出库单订单号
                $soOrderNums = trim($res['data']['soOrderNum'], ',');
                $soOrderNums = explode(',', $soOrderNums);
                $soOrderNum = $soOrderNums[0];//防止有多个出库单号情况
                //如果出库单为空也返回失败
                if (!empty($soOrderNum)) {
                    $upRes = $order->update(['soOrderNum'=>$soOrderNum], ['order_id'=>$sdf['order_id']]);
                    if (!$upRes) {
                        $msg = '更新WMS出库单订单号时出错';
                        return false;
                    }
        ...
        $order->fireEvent('shipping', $aUpdate, $sdf_order['member_id']);//发送短信提醒
        ...
        if ($stage)
            $is_change_point = $obj_reducte_point->change_point($sdf_order['member_id'], 0 - intval($sdf_order['score_u']), $msg, 'order_pay_use', 1, $sdf['order_id'], $controller->user->user_id, $reason);
        ...
        if ($stage)
            $obj_add_point->change_point($sdf_order['member_id'], intval($sdf_order['score_g']), $msg, 'order_pay_get', 2, $sdf['order_id'], $controller->user->user_id, $reason);
