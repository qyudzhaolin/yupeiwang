...
class b2c_order_total
{
    ...
    public function order_total_method(&$controller,$cart,$sdf_order,$need_pmt_info='false',$view='site/cart/checkout/checkout_total.html')
    {
        ...
        //预售定金加运费
        if($payment_detail['prepare']){
            $b=$payment_detail['prepare'];
            $a=$payment_detail['cost_freight']+$b['preparesell_price'];
            $payment_detail['prepare']['preparesell_price']=$a;
        }
        // ee($payment_detail);
        ...
        if (!empty($cart['contract_goods'])) {
            $controller->pagedata['acart'] = $cart;
        }
    ...
    public function payment_detail(&$controller,$cart,$sdf_order)
    {
        ...
        if($sdf_order['shipping_id'])
        {
            ...
            if($dlytype_info['costby']=="weight"){
               ...
               $cost_freight = @utils::cal_fee($dlytype_info['dt_expressions'],$cart_info['items_quantity'] ,$cost_item,$dlytype_info['firstprice'],$dlytype_info['continueprice']);//配送费
            }
