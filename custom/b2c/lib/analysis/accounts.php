...
class b2c_analysis_accounts extends ectools_analysis_abstract implements ectools_analysis_interface
{
    protected $charts_action = 'chart_view_accounts';//账款
    ...
    public $targets = array(
        'accounts_order' => array(   
            'name' => '订单统计',
        ),
        'accounts_sales' => array(
            'name' => '销售额统计',
        ),
        'price_cost' => array(
            'name' => '应收应付统计',
    ...
    public function ext_detail(&$datas){
        $filter = $this->_params;
        $statDatas = $this->app->model('statdata')->get_stat_datas($filter,['accounts','scale']);
        $datas = [
            '应收总额'=>[
                'value'=>$statDatas['tobeChargedAmount'],
                'memo'=>'应收总额',
                'icon'=>'coins_stop.gif',
            ],
            '已收总额'=>[
                'value'=>$statDatas['chargedAmount'],
                'memo'=> '已收总额',
                'icon'=>'coins.gif',
            ],
            '应付总额'=>[
                'value'=>$statDatas['costAmount'],
                'memo'=> '应付总额',
                'icon'=>'coins_go.gif',
            ],
        ];
    ...
    {
        return array(
            'model' => 'b2c_mdl_analysis_accounts',
            'params' => array(
                'actions'=>array(
                    array(
                        ...
                        'class'=>'export',
                        'icon'=>'add.gif',
                        'href' => 'index.php?app=importexport&ctl=admin_export&act=export_view&_params[app]=b2c&_params[mdl]=b2c_mdl_analysis_accounts',
                        'target'=>'{width:400,height:170,title:\''.app::get('b2c')->_('生成报表').'\'}'),
                ),
                'title'=>app::get('b2c')->_('账款统计'),
