...
class b2c_cart_object_goods implements b2c_interface_cart_object{
     ...
     * @return mixed array 处理后的数据
     */
    public function get_data_contract($params=array(),&$msg='')
    {
        if (!$params) return array();
        return $params;
    }    
     ...
     * @return boolean
     */
    public function add_object_contract($aSave, &$msg='', $append=true,$is_fastbuy=false)
    {
        if (empty($aSave)) {
            $msg = app::get('b2c')->_('合约商品参数错误！');
            return false;
        }

        $aSave['member_ident'] = $this->member_ident;
        $aSave['obj_type']     = 'goods';

        foreach ($aSave['goods'] as $key => $aData) {
            $aSave['goods'][$key]['obj_ident'] = $this->_generateIdent($aData);
            $aSave['goods'][$key]['quantity'] = floatval(($aData['num']?$aData['num']:1));
        }
        if($is_fastbuy){
            $aSave['is_fastbuy'] = 'true';
        }
        $is_save = $this->oCartObjects->save_contract($aSave);
        if (!$is_save){
            $msg = app::get('b2c')->_('购物车保存失败！');
            return false;
        }
    ...
    public function getAll($rich = false,$is_fastbuy=false) {
        if($this->get_cart_status())  {
            ...
            //合约
            if ($this->is_contract()) {
                $this->member_ident = $this->get_md5_ident($this->arr_member_info['member_id']);
            }
            $filter = array('obj_type' => 'goods','member_ident'=> $this->member_ident);
            if($is_fastbuy){
                $filter['is_fastbuy'] = 'true';
            }
            //$this->oCartObjects = b2c_mdl_cart_objects
            $aResult= $this->oCartObjects->getList('*',$filter);
            // ee($filter);
        }
        if(empty($aResult)) {
    ...
    public function deleteAll() {
        $wheres = ['member_ident'=>$this->member_ident, 'obj_type'=>'goods'];
        //如果是合约则只清空合约的购物车#20180907#
        if ($this->is_contract()) {
            $wheres['buy_type'] = 'contract';
        }
        return $this->oCartObjects->delete($wheres);
    }
    ...
    private function _check($aData, $_check_adjunct=true) {
        ...
        if($aData['quantity'] < 1) return $this->get_error_msg( '购买数量不能比1小' );
    ...
    function _get_products($aProductId) {
        ...
        ///////////////// 货品信息 ///////////////////////
        $fields = 'g.spec_desc as spec_desc_goods,p.product_id,p.goods_id,p.bn,g.score as gain_score,
                   p.cost,p.name, p.store, p.marketable, g.params, g.package_scale, g.package_unit, 
                   g.package_use, p.freez,g.goods_type, g.nostore_sell, g.min_buy,g.type_id,g.cat_id,
                   g.image_default_id,p.spec_info,p.spec_desc,p.price,p.weight,p.unit,p.sales_price,p.unit gunit,
                   t.setting, t.floatstore,p.storehouse_id,p.owner_code,p.sku_code,p.is_yp_store,p.unitEn';
        $sSql = "SELECT 
                     {$fields}
    ...
    //是否是合约
    private function is_contract( $str='' ) {
