...
class b2c_finder_orders{
    ...
    public function __construct($app)
    {
        ...
        //#TODO#
        $this->promotion_type_id = $_GET['view'];
        if ($this->promotion_type_id != '15') {
            unset($this->column_order_fund,$this->column_active_num);//非预售不显示
        }
    }

    //预售
    var $column_order_fund = "当前已完成";
    function column_order_fund($row){
        if($this->promotion_type_id != '15') return;//非预售不显示
        $o =  app::get('preparesell')->model('order_fund');
        $sql2 = "SELECT  * from  sdb_preparesell_order_fund where `status` = 'finish' and order_id=".$row['order_id']." ORDER BY begin_time desc LIMIT 1 " ;
        $num = $o->db->select($sql2);

        $funds=['y'=>'预付款','z'=>'中期进度款','w'=>'尾款','o'=>'其他'];
        $currentFundName = '';
        if($num){
            foreach ($num as $key => $value) {
                $currentFundName = isset($funds[$value['fund_name']]) ? $funds[$value['fund_name']] : $funds['o'];
            }
        }
    ...
    // 虚拟列预售活动号
    var $column_active_num = "预售活动号";
    function column_active_num($row){
        if($this->promotion_type_id != '15') return;//非预售不显示

        $o =  app::get('preparesell')->model('preparesell');
        $sql2 = "select  p.active_num as active_num,o.order_id as order_id  from  sdb_preparesell_preparesell as p,sdb_preparesell_prepare_order as o
           where o.prepare_id=p.prepare_id and o.order_id=".$row['order_id'];
        $num = $o->db->select($sql2);
        if($num){
            foreach ($num as $key => $value) {
                 $num[$key]['active_num']=$value['active_num'];
            }

        }
        return    $num[0]['active_num'];
    }


    //合约号
    var $column_contract_no = "合约号";
    function column_contract_no($row){
        if($this->promotion_type_id != '18' || empty($row)) return;//非合约订单不显示
        $o =  app::get('b2c')->model('contract_order');
        $contactOrderRow = $o->getRow('*',['order_id'=>$row['order_id']]);
        if($contactOrderRow){
            return $contactOrderRow['contract_no'];
        }
        return '';
    }

    //是否出库
    var $column_is_out_order = "出库";
    function column_is_out_order($row){
        if($this->promotion_type_id != '18' || empty($row)) return;//非合约订单不显示
        $o =  app::get('b2c')->model('contract_order');
        $contactOrderRow = $o->getRow('*',['order_id'=>$row['order_id']]);
        if(!empty($contactOrderRow)){
            return $contactOrderRow['step_type'] == 'out' ? '是' : '否';
        }
        return '';
    }
    ...
    public function detail_basic($order_id)
    {
        ...
        if (substr($_POST['finish'], 0, 3) == 'pay')
        {
    ...
    public function detail_bills($order_id)
    {
        ...
        if ($aBill)
        {
            ...
            {
                $arr_payments = $payments->dump($str_bill['bill_id'], 'pay_name,status,t_payed,fund_type,trade_no,bank_name,bank_no');
                $str_bill['pay_name'] = $arr_payments['pay_name'];
                $str_bill['status'] = $arr_payments['status'];
                $str_bill['t_payed'] = $arr_payments['t_payed'];
                $str_bill['trade_no'] = $arr_payments['trade_no'];
                $str_bill['bank_name'] = $arr_payments['bank_name'];
                $str_bill['bank_no'] = $arr_payments['bank_no'];
                if ($arr_payments['fund_type']=='y') {
                $str_bill['fund_type'] = '预付款';
                }elseif ($arr_payments['fund_type']=='z') {
                $str_bill['fund_type'] = '中期进度款'; 
                }elseif ($arr_payments['fund_type']=='w') {
                $str_bill['fund_type'] = '尾款'; 
                }
            }
        }
        ...
        if ($aRefund)
        {
            ...
            {
                $arr_refunds = $refunds->dump($str_refund['bill_id'], 'pay_name,status,t_payed,fund_type');
                $str_refund['pay_name'] = $arr_refunds['pay_name'];
                $str_refund['status'] = $arr_refunds['status'];
                $str_refund['t_payed'] = $arr_refunds['t_payed'];
                 if ($arr_refunds['fund_type']=='y') {
                $str_refund['fund_type'] = '预付款';
                }elseif ($arr_refunds['fund_type']=='z') {
                $str_refund['fund_type'] = '中期进度款'; 
                }elseif ($arr_refunds['fund_type']=='w') {
                $str_refund['fund_type'] = '尾款'; 
                }
            }
        }
    ...
    public function detail_delivery($order_id){
        ...
        $consign = $objDelivery->getList('*',array('order_id'=>$order_id,'status|ne'=>'cancel','disabled'=>'false'));
        $order = $objOrders->getRow('*',$filter);
        ...
        //如果是宇培管库则出库单号为WMS出库单号，也就是用order表的soOrderNum号替换delivery_id号
        if ($order && !empty($order['soOrderNum'])) {
            $cloudLink=getConfig('cloudLink');//云平台链接地址
            $ctime = time();
            foreach ($consign as $key => $value) {
                $consign[$key]['delivery_id'] = $order['soOrderNum'];
                $consign[$key]['logistictrack_url'] = "{$cloudLink['wms_url']}?flag=ddgz&ordernum={$value['logi_no']}&t={$ctime}";
            }
        }
        // ee($consign);
        $render->pagedata['consign'] = $consign;
    ...
    public function detail_logs($order_id){
        ...
        if($data){
            foreach ($data as $k=>$v) {
               if ($v['behavior']=='confirm') {
                         $data[$k]['behavior']='财务确认';
                   if($v['result']=='SUCCESS'){
                        $data[$k]['log_text']='订单财务确认成功！';
                    }else{
                        $data[$k]['log_text']='订单财务确认失败！';  
                     }
    ...
    {   
        ...
        if( $this->promotion_type_id==19){
           $actions['sequence']['finance']=array(
            'label' =>'财务确认', 
            'flow' =>'', 
            'disable' =>'', 
            'app' =>'b2c', 
            'act' =>'finance', 
            'confirm' => '确认操作会将该订单到待发货，确认要执行吗?'
             );
         }
    ...
    public function column_printer($row)
    {
        ...
        $html.='<div class="span-auto"><a href="index.php?app=b2c&ctl=admin_order&act=printing&p[0]=1&p[1]=' . $row['order_id'] . '" title='.app::get('b2c')->_("采购单").' target="_blank">'.app::get('b2c')->_('采').'</a></div><div class="span-auto">
                            <a href="index.php?app=b2c&ctl=admin_order&act=printing&p[0]=2&p[1]=' . $row['order_id'] . '" title='.app::get('b2c')->_("销售单").' target="_blank">'.app::get('b2c')->_('销').'</a></div><div class="span-auto">
                              <a href="index.php?app=b2c&ctl=admin_order&act=printing&p[0]=4&p[1]=' . $row['order_id'] . '" title='.app::get('b2c')->_("签收单").' target="_blank">签</a></div><div class="span-auto last">
                            ';
