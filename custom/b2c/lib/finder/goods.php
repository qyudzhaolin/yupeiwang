...
class b2c_finder_goods{
    ...
    //配送范围列
    var $column_goods_shiparea = "配送范围";
    function column_goods_shiparea($row){
        $o =  app::get('b2c')->model('shiparea');
        $num = $o->count(['goods_id'=>$row['goods_id']]);
        return $num >0 ? '<span class="c-green">已设置</span>' : '<span class="c-orange">未设置</span>';
    }
