...
class b2c_user_passport
{
    ...
    public function pre_account_signup_process($accountData,$password_account=null){
        ...
        $login_password = pam_encrypt::get_encrypted_password(trim($accountData['login_password']),'member',$use_pass_data);
          if($accountData['user_name']!=null){
             $use_pass_data['login_name'] =$accountData['user_name'];

         $login_password2 = pam_encrypt::get_encrypted_password(trim($accountData['login_password']),'member',$use_pass_data);
        }
    ...
    public function save_members($saveData,&$msg){
        ...
        if($saveData['pam_account']['user_name']!=null){
              $saveData['pam_account']['member_id'] =$member_id;
              $saveData['pam_account']['login_type']='local';
              $saveData['pam_account']['login_password']= $saveData['pam_account']['login_password2'];
              $saveData['pam_account']['login_account']=$saveData['pam_account']['user_name'];
              $saveData['pam_account']['password_account']=$saveData['pam_account']['user_name'];
            if(!app::get('pam')->model('members')->save($saveData['pam_account'])){
                $db->rollBack();
                return false;
            }
