...
class b2c_member_addrs{
  ...
  //保存修改编辑后的收货地址
	public function purchase_save_addr($arr_post=array(),$member_id,&$msg=''){
        //检查必填信息是否存在
          $specialArr1 = $arr_post['_DTIME_'];
          if(count($specialArr1) > 1){
             $h=$specialArr1['H'];
             $m=$specialArr1['M'];
             $hm=$h['special'].":".$m['special']."到".$h['special1'].":".$m['special1'];
            }
        ...
        if($arr_post['day'] == 'special'){
            if( $this->isDate( $arr_post['special'] ) ){
                $save_data['day'] = $arr_post['special'];
                 $save_data['time'] = $hm;
            }else{
                $specialArr  = explode(' ',$arr_post['special']); //指定日期
    ...
    function check_addr_post($arr_post,$member_id,&$msg=''){
        ...
        if(!trim($arr_post['addr'])){
            $msg = app::get('b2c')->_('收货地址未填写');
			return false;
        }
        if(!trim($arr_post['name'])){
        ...
        if(!$arr_post['mobile'] && !$arr_post['tel']){
         ...
         if(count($specialArr1) > 1){
            $time1=date("Y-m-d");
            $time2=$arr_post['special'];
             $h=$specialArr1['H'];
             $m=$specialArr1['M'];
             $hm=$h['special'].":".$m['special']."到".$h['special1'].":".$m['special1'];
            if($hm&&$time2){
            if(strtotime($time2)-strtotime($time1)<0){
            $msg = app::get('b2c')->_('当前配送日期应该大于现在日期');
            return false;
            }
            }
        }
