...
class b2c_ctl_site_cart extends b2c_frontpage{
    ...
    //添加合约商品出库(合约商品立即购买,类似于修改之前的add方法)
    public function add_contract($type='goods',$step_type='other') {
        if (!in_array($step_type, ['out','other'])) {
            $this->apiReturn(['error'=>1,'message'=>'阶段类型错误！']);

        }
        $data = $this->_request->get_params(true);
        define('BUYTYPE', 'contract');
        $_SESSION['contract_id'] = $contract_id = trim($data['contract_id']);
        $_SESSION['step_id'] = $step_id = trim($data['step_id']);
        $_SESSION['step_type'] = trim($step_type);//出库类型


        //该非出库阶段是否已经生成过订单了？
        if ($step_type == 'other') {
            $mdl_contract = app::get('b2c')->model('contract');//合约主表
            $orderRows = $mdl_contract->getContractOrders($contract_id,['step_id'=>$step_id]);
            if (!empty($orderRows)) {
                $orderRow = $orderRows[0];//取第一个
                $pay_status = $orderRow['pay_status'];
                if ($pay_status == '0') {
                    //未支付
                    $order_id = $orderRow['order_id'];
                    $jumpUrl = $this->gen_url(array('app'=>'b2c','ctl'=>'site_paycenter','act'=>'index','arg0'=>$order_id));
                    $this->apiReturn(['error'=>2,'message'=>$jumpUrl]);
                }

                //已支付
                if ($pay_status == '1') {
                    //未支付
                    $this->apiReturn(['error'=>1,'message'=>'请勿重复付款！']);
                }
            }
        }else{
            $_SESSION['extra'] = isset($data['extra']) ? trim($data['extra']) : '0';//是否包含临时附加费
        }
        ...
        if (!$arr_objects[$type])
        {
            $this->apiReturn(['error'=>1,'message'=>'商品类型错误！']);
        }
        $aData = $data;
        $aData['apply_platform'] = '1';

        $obj_cart_object_goods = kernel::single('b2c_cart_object_goods');
        $obj_ident = $obj_cart_object_goods->add_object_contract($aData, $msg,true);

        if(!$obj_ident){
            $this->apiReturn(['error'=>1,'message'=>$msg]);

        } else {
            $jumpUrl = $this->gen_url(array('app'=>'b2c','ctl'=>'site_cart','act'=>'checkout','arg0'=>'false','arg1'=>'contract','arg2'=>$step_type));
            $this->apiReturn(['error'=>0,'message'=>$jumpUrl]);
        }
    }
     ...
     * @return void
     */
    public function checkout($isfastbuy=0,$buyType='',$step_type='')
    {
        /**
         * 取到扩展参数
         */
        ...
        $this->pagedata['buyType'] = $buyType;
        $this->pagedata['step_type'] = $step_type;//出库类型
        if (!$this->member_status){
          if($isfastbuy){
            $url = $this->gen_url(array('app'=>'b2c','ctl'=>'site_cart','act'=>'loginBuy','arg0'=>'true'));
        ...
        // 初始化购物车数据
        $this->_common(false,$isfastbuy,$buyType);
        $this->begin(array('app'=>'b2c','ctl'=>'site_cart','act'=>'index'));
    ...
    public function checkout_result($isfastbuy=0){
        ...
        //是否选择启用账期支付
        if($arrMember){
         $accountpay_m = $this->app->model('members');
         $accountpay = $accountpay_m->getRow('accountpay',array('member_id'=>$arrMember['member_id']));
           $this->pagedata['is_accountpay'] = $accountpay['accountpay'];
        }
        //是否发票信息已经填写过
         if($arrMember['member_id']){
          $obj_order = $this->app->model('orders');
          $sql="SELECT order_id,member_id,tax_content,is_tax,tax_type,cost_tax,tax_company,tax_code,tax_bank,bank_account,tax_addr,tax_remark from ".$obj_order->table_name(true)."  where member_id=".$arrMember['member_id']." and is_tax=".true." ORDER BY createtime desc LIMIT 1" ; 
           $invoice= $obj_order->db->select($sql);

           if($invoice){
            $this->pagedata['invoice'] = $invoice;
            $this->pagedata['isinvoice'] =1;
           }
        }
        ...
        if($preparesell_is_actived == 'true'){
            ...
            if($arrproducts){
               $this->pagedata['is_prepare'] =$arrproducts['promotion_type'];
            }
        }
        ...
        $this->pagedata['def_addr'] = $def_addr;
         ...
         // $cityselect=$_COOKIE['shiparea'];
         // if($cityselect){
         //      $sql="select * from sdb_ectools_regions  WHERE region_id=".$cityselect ; 
         //      $result= $obj_order->db->select($sql);
         //       $sql1="select * from sdb_ectools_regions  WHERE region_id=".$result[0]['p_region_id'] ; 
         //   $result= $obj_order->db->select($sql);
         //   $result1= $obj_order->db->select($sql1);
         //  $cityselect="mainland:".$result1[0]['local_name']."/".$result[0]['local_name'].":2";
         //  if(empty($def_addr)){
         //    $this->pagedata['is_select'] =1;
         //    $this->pagedata['cityselect'] =$cityselect ;
         //  }
         // }
       ...
       /* $objproducts = $this->app->model('products');
        ...
        {
            //预售#TODO#
            $_nums = $this->pagedata['aCart']['object']['goods'][0]['quantity'];
            $pre=$prepare->getSpecialProduct($product_id,$_nums);
    ...
    public function integral_update(){
            ...
            $this->page('site/cart/checkout/index.html');
    }
    ...
    public function order_point_discount_html()
    {
        //这里还欠缺一个total_amount
        // 初始化购物车数据
        $this->mCart = $this->app->model('cart');
        $aData = $this->_request->get_params(true);
        $aData['is_fastbuy'] = true;
        $aData['apply_platform'] = '1';
        ...
        if ($obj_point_dis){
            $obj_point_dis->set_order_total($total_amount);
            $point_dis_html = $obj_point_dis->get_html($member_id);
            $point_dis_js = $obj_point_dis->get_javascript($member_id);
        }

        echo $point_dis_html;
        echo '<script>'.$point_dis_js.'</script>';
        exit;
    }
    ...
    //配送方式根据送货地址联动
    public function delivery_change($buyType=''){
        $isfastbuy = $_POST['isfastbuy'];
        $this->set_header();
        if ($buyType == 'contract') {
            define('BUYTYPE', 'contract');
            $this->_common(false,false,$buyType);
        }else{
            $this->_common(false,$isfastbuy,$buyType);//#TODO#20180713,修改切换地址运费显示bug
        }
    ...
    public function payment_change(){
        ...
        {
             $this->pagedata['is_prepare'] ='prepare';
            $sdf['no_offline']='true';
            $this->pagedata['has_cod'] = 'false';
        }
        $currency = app::get('ectools')->model('currency');
        ...
        if($arrMember){
         ...
         if($_COOKIE['is_contract']){
         $this->pagedata['is_contract'] = "是";
    ...
    public function total($buyType=''){
        if ($buyType == 'contract') {
            define('BUYTYPE', 'contract');
            $this->pagedata['buyType'] = 'contract';
        }
        $this->_common(false,$_POST['isfastbuy'],$buyType);
    ...
    function favorite($nPage=1){
        ...
        if ($this->check_login()) {
            ...
            $aData = kernel::single('b2c_member_fav')->get_favorite($this->app->member_id,$membersData['members']['member_lv_id'],$nPage,5);
        ...
        $this->page('site/cart/favorite.html', true);
    }
    ...
    public function _common($flag=0,$is_fastbuy=false,$buyType='') {
        // 购物车数据信息
        $aData = $this->_request->get_params(true);
        if($is_fastbuy){
        ...
        if ($buyType == 'contract') {
            $aCart = $this->mCart->get_objects($aData);
            //这会重新构造cart里面的[合约商品每种货品价格列表计算价格 && 单价 && 总价]
            $mdl_contract = app::get('b2c')->model('contract');//合约主表
            $member_id = kernel::single('b2c_user_object')->get_member_id();
            $step_type = $_SESSION['step_type'];
            if ($step_type == 'out') {
                $mdl_contract->editContractCart($aCart,$member_id);
            }else{
                $mdl_contract->editContractVirtualCart($aCart,$member_id);
            }
            //合约下无账期支付
             $this->pagedata['is_contract'] ="是";
              setcookie("is_contract", "是" );

        }else{
            $aCart = $this->mCart->get_objects($aData);//$this->mCart = b2c_mdl_cart
        }
        $this->_item_to_disabled( $aCart,$flag ); //处理购物扯删除项
        $this->pagedata['aCart'] = $aCart;
        // ee($aCart);
        // ee($aCart['object']['goods']);
