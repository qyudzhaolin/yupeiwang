...
class b2c_ctl_site_gallery extends b2c_frontpage{
    ...
    public function index($cat_id='',$urlFilter=null,$orderBy=0,$page=1,$virtual_cat_id=null,$showtype=null,$cid=null) {
        ...
        // $cid=11;
        if($cid){
          $params['filter']['cid']=$cid;
              $gprovenance = app::get('b2c')->model('gprovenance');
              $site_c= $gprovenance->getRow('gcountry_of_origin',array('provenance_id'=>$cid));
              $this->pagedata['select_c'] = $site_c['gcountry_of_origin'];
              $this->pagedata['is_limit'] =1;
        }
...
/*对商品标题进行字符控制*/
        //echo '<pre>';print_r($goodsData);exit();
         ...
         $gprovenance = app::get('b2c')->model('gprovenance');
         $sql="select provenance_id,gcountry_of_origin FROM sdb_b2c_gprovenance where 1 GROUP BY  gcountry_of_origin ORDER BY provenance_id asc";
         $country =$gprovenance->db->select($sql);
         $screen['screen']['country']=$country;
                // ee($screen['screen']);
        ...
        if($cat_id){
        $goods_cat = $goods_cat->getRow('cat_id,cat_name', array('cat_id'=>$cat_id));  
        }
        $this->pagedata['goods_cat'] = $goods_cat;
...
//        $_POST['virtual_cat_id'] = utils::_RemoveXSS($_POST['virtual_cat_id']);
        //实际上ajax刷新时无需考虑虚拟分类的问题，把虚拟分类当做一个搜索的快捷方式看待就可以了。 -by summer
        $_POST['virtual_cat_id'] = null;
        ...
        $gprovenance = app::get('b2c')->model('gprovenance');
        if($_POST['provenance_id']){
            foreach ($_POST['provenance_id'] as $key => $value) {
                    $country_name = $gprovenance->getRow('gcountry_of_origin',array('provenance_id'=>$value));
                if($country_name){
                    $sql=" SELECT  provenance_id  from  sdb_b2c_gprovenance  WHERE gcountry_of_origin='".$country_name['gcountry_of_origin']."'";
                    $provenance_id=$gprovenance->db->select($sql);
                   $all_id[]= $provenance_id;
            ...
            if($all_id){
                foreach ($all_id as $k => $v) { 
                      $v_id=$v;
                     foreach ( $v_id as $k1 => $v2) {
                      $all_countryid[]=$v2['provenance_id'];
                     }
        ...
        //重新构建结果
        $_POST['provenance_id']=$all_countryid;
        }
        ...
        if($goodsData){
            ...
            //后台站点设置搜索为空页面
            $a='<h1 class="error" style="text-align: center;font-size: 25%; margin-top: 100px;">非常抱歉，没有找到相关商品</h1>
        <p style="margin:15px 1em;"></p>';
    ...
    public function get_goods($filter,$page=1,$orderby){
        ...
        // $this->app->model('shiparea')->makeGoodsfilterByShiparea($filter);//配送区域过滤
        //原产国过滤
         $gprovenance = app::get('b2c')->model('gprovenance');
        if($filter['cid']){
        
                    $country_name = $gprovenance->getRow('gcountry_of_origin',array('provenance_id'=>$filter['cid']));
                if($country_name){
                    $sql3=" SELECT  provenance_id  from  sdb_b2c_gprovenance  WHERE gcountry_of_origin='".$country_name['gcountry_of_origin']."'";
                    $provenance_id=$gprovenance->db->select($sql3);
                   $all_id[]= $provenance_id;
        ...
        //重新构建结果
        $filter['provenance_id']=$all_countryid;
        }
        //  //对内对外过滤
        ...
        //  $accountpay = $accountpay_m->getRow('customertype',array('member_id'=>$arrMember['member_id']));
        //   if($accountpay['customertype']=='对外客户'){
        //     $filter['sales_mode']=1;
           
        //   }elseif($accountpay['customertype']==''){
        //      $filter['sales_mode']=1;
        ...
        // }
         //预售信息过滤
        $sql =" select  g.goods_id from  sdb_b2c_goods as g,sdb_b2c_products as 
              p,sdb_preparesell_preparesell_goods as pg 
              where  g.goods_id=p.goods_id and p.product_id=pg.product_id GROUP BY g.goods_id ";
        $results = $goodsModel->db->select($sql);
        // ee($results);
        $a=$filter['goods_id|in'];
        // ee($a);
        ...
        //库存不足过滤
        $nostore_sell="'0'";
        $sql2="select g.goods_id from  sdb_b2c_goods as g,sdb_b2c_products as p where  g.goods_id=p.goods_id and g.store=0 and  g.nostore_sell=". $nostore_sell." GROUP BY g.goods_id";
         $nostoreresults = $goodsModel->db->select($sql2);
         foreach ($a as $key => $value) {
            foreach ($nostoreresults as $kn => $vn) {
                if($value==$vn['goods_id']){
                   unset($a[$key]);
                }
            }
        ...
        if (empty($a)) {
            $a[201805221412] = [201805221412];
        }
        // $filter['goods_id|in']=$a;
