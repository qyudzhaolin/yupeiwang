...
class b2c_ctl_site_paycenter extends b2c_frontpage{
     ...
     *
     * */
    public function index($order_id,$newOrder=false,$order_fund_id=0)
    {
        //结算阶段
        if (isset($_SESSION['step_type'])) {
            $this->pagedata['step_type'] = $_SESSION['step_type'];
        }
        ...
        //#TODO#
        if($sdf['promotion_type']=='prepare'){
            $fundNames=['y'=>'支付预付款','z'=>'支付中期进度款','w'=>'支付尾款'];
            /*获取该付哪个款项(基本逻辑：先判断该付哪个款项，拿到合法的order_fund_id)*/
            //预售订单款项信息
            $orderFundModel = app::get('preparesell')->model('order_fund');
            $fundRow=$orderFundModel->getOrderFund($order_fund_id);
            if (empty($fundRow)) {
                $this->begin();
                $this->end(false,  app::get('b2c')->_('无效的订单款项！'), $this->gen_url(array('app'=>'site','ctl'=>'default','act'=>'index')));
            }
           if($fundRow['fund_name']=='y'){
             //预售加运费
             $order = app::get('b2c')->model('orders');
             $order_status['order_id'] = $fundRow['order_id'];
             $order_status['promotion_type'] ='prepare';
             $order_status['status'] = 'active';
             $aData = $order->getRow('cost_freight',$order_status);
          
            if($aData){
               $fundRow['totalPayment'] =$aData['cost_freight']+$fundRow['totalPayment'];
            }
           }
           
             $this->pagedata['fundRow']=$fundRow;
            //支付按钮显示名称
            $this->pagedata['buttonName']=isset($fundNames[$fundRow['fund_name']]) ? $fundNames[$fundRow['fund_name']] : '去付款';
            $this->pagedata['order_fund_id']=$order_fund_id;
            $sdf['cur_money'] = $fundRow['totalPayment'];
        }
        ...
        if($sdf['promotion_type']=='prepare'){
            $this->pagedata['order']['total_amount'] = $this->pagedata['order']['cur_money'];
        }else{
    ...
    public function dopayment($pay_object='order',$combination_pay = false)
    {
        ...
        if($_POST['payment']['def_pay']['pay_app_id']=='cpcnquick'){

            if(!$_POST['TxSNBinding']){
                echo '<script>alert("未选择银行卡！")</script>';exit;
            }
        }
        ...
        if ($pay_object)
        {
                ...
                case 'order':
                    ...
                    if($pay_object == 'order' && (!isset($_POST['payment']['other_online']) || $combination_pay == true) ){
                        ...
                        if($promotion_type=='prepare'){   
                            //预售订单款项信息
                            $orderFundModel = app::get('preparesell')->model('order_fund');
                            $fundRow=$orderFundModel->getOrderFund($sdf['order_fund_id']);

                            $orders['total_amount'] = $objMath->number_div(array($orders['cur_amount'], $orders['cur_rate']));
                            $sdf['money']        = $fundRow['totalPayment'];
                            $sdf['currency']     =$orders['currency'];
                            $sdf['cur_money']    = $fundRow['totalPayment'];
                            $sdf['cur_rate']     = $orders['cur_rate'];
                            $sdf['fund_type']    = $fundRow['fund_name'];
                            $sdf['fund_id']      =$fundRow['fund_id'];
                        }else{
                            //以前的
                    ...
                    if($order_payed>0){
                        //查看该款项是否已经支付
                        if($promotion_type=='prepare'){
                            $fundPaystatus=$orderFundModel->orderFundPaystatus($sdf['order_id'], $sdf['order_fund_id']);
                            if($fundPaystatus['status'] != 0){
                                $this->splash('failed',null, $fundPaystatus['msg']);
                            }
                        }
                        if($order_payed==$orders['total_amount']){
                            $this->splash('failed',null, app::get('b2c')->_('您已经支付过了无需再支付'));
                        }
                        $payedMoney = $order_payed+$_POST['payment']['def_pay']['cur_money'];
                        if((string)$payedMoney > $orders['total_amount']){
            ...
            if ($sdf['pay_app_id']=='cpcnquick'){
                $sdf['payinfo']['TxSNBinding'] = $_POST['TxSNBinding'];
            }
             //中金预售钱处理
             if($promotion_type=='prepare'){
                 if($sdf['fund_type']=='y'){
                  ...
                  $aData = $order->getRow('cost_freight',$order_status);
                     ...
                     if($aData){
                       $sdf['total_amount'] =$aData['cost_freight']+$sdf['money'];
                     }
                 }else{
    ...
    {
       $payobj = kernel::single('ectools_payment_plugin_cpcnquick');
       $param = array('code' => $_POST['code'], 'pamentNo' => $_POST['PaymentNo'],'money'=>$_POST['money'],'return_url'=>$_POST['return_url']);
       $res=$payobj->pay($param);exit();
    }
