...
class b2c_ctl_site_order extends b2c_frontpage{
    ...
    public function create()
    {
        ...
        //这是合约购物车有数据的重要依据
        if ($arr_args['post']['buyType'] == 'contract') {
        ...
        $chkRes = $mdl_orders->checkYPstoreWhenBuy($aCart);
        if ($chkRes['error']) {
            $this->end(false, $chkRes['message'], '',true,true);
        }

        //购物车城市商品匹配校验
        if($aCart){
            $goods_id=$aCart['goods_min_buy'];
            if($goods_id){
                 foreach ($goods_id as $key => $value) {
                 $buy_goodsid[$key]=$key;
                 }
             }
            if($_POST['buyType']!='contract'){
        //根据地区id获取商品结果集
           $address_id = json_decode($_POST['address'],true);
             $regions = app::get('ectools')->model('regions');
             $regions_status['region_id'] =$address_id['area'];
             $aData = $regions->getRow('p_region_id,region_grade',$regions_status);
             if($aData['region_grade']==3){
              $address_id['area']=$aData['p_region_id'];
             }
            $all_goodsid  =  $this->app->model('shiparea')->getGoodsIdsByregionId($address_id['area']);
            $result=array_diff($buy_goodsid,$all_goodsid);
            if($result){
             $msg .= app::get('b2c')->_("您的购物车中有它城市的商品,与当前收货城市不匹配，请检查购物车。")."<br />";  
        ...
        if(!$_POST['address']){
            ...
            // //收货城市是否与首页确认的收货城市匹配
            // $isMatch  =  $this->app->model('shiparea')->receiveAddressIsMatchShiparea($address['area']);
            // if ($isMatch['error']) {
            //     $msg = $isMatch['msg'];
            //     $this->end(false, $msg,'reload',false,false);
            // }
        ...
        //在订单创建前 看存在积分使用的情况下 积分功能是否开启 以及积分进行兑换或者抵扣时做相应的积分检查
        $rs_used_point = $this->check_order_used_point($_POST,$aCart,$point_msg);
        if(!$rs_used_point){
            $db->rollback();
            $this->end(false, $point_msg, '',true,true);
        }

        if (defined('BUYTYPE') && BUYTYPE == 'contract') {
        ...
        //$obj_order_create = b2c_order_create
        // ee($aCart);
        ...
        if (defined('BUYTYPE') && BUYTYPE == 'contract') {
            //合约订单标记
            $order_data['promotion_type']='contract';

            //这会重新构造order_data里面的总价(cost_item、total_amount、cur_amount、amount)
            $contractOutTotalAmount = $aCart['contract_goods']['total'];//当前合约出库批次总价
            $order_data['cost_item'] = $contractOutTotalAmount;
            $order_data['total_amount'] = $contractOutTotalAmount;
            $order_data['cur_amount'] = $contractOutTotalAmount;
            foreach ($order_data['order_objects'] as $okey => $oval) {
                $product_id = $oval['order_items'][0]['products']['product_id'];
                $order_data['order_objects'][$okey]['order_items'][0]['amount'] = $aCart['contract_goods']['outDetial'][$product_id]['amount'];
            }
        }
        // ee($aCart);
        ...
        {   $data=array(
            ...
            $order_fund_id = kernel::service('prepare_order')->sava_prepare_order($data);
        }


        if ($result && defined('BUYTYPE') && BUYTYPE == 'contract') {
            $step_type = $_SESSION['step_type'];
            if ($step_type == 'out') {
                //生成合约商品出库流水
                $outStoreRes = $mdl_contract->saveOutStore($aCart['contract_goods'],$order_data['order_id']);
                if ($outStoreRes['error']) {
                    $db->rollback();
                    $this->end(false, $outStoreRes['message'], '',true,true);
                }
            }

            
            //生成合约订单
            $contract_goods = $aCart['contract_goods'];
            $saveOutStoreRes = $mdl_contract->createContractOrder($contract_goods,$order_data['order_id']);
            if ($saveOutStoreRes['error']) {
                $db->rollback();
                $this->end(false, $saveOutStoreRes['message'], '',true,true);
            }
        }
        ...
        if ($result)
        {
            ...
            $order_fund_id = isset($order_fund_id) ? $order_fund_id : '';
            $this->end(true, $this->app->_("订单生成成功！"), $this->gen_url(array('app'=>'b2c','ctl'=>'site_paycenter','act'=>'index','arg0'=>$order_id,'arg1'=>'true','arg2'=>$order_fund_id)),'',true);
        }
        else
            $this->end(false, $msg, $this->gen_url(array('app'=>'b2c','ctl'=>'site_cart','act'=>'checkout')),true,true);
    }
    ...
    //在提交订单操作创建订单的这个步骤：检查在选择积分兑换或者抵扣的情况下必须是有这部分有效的积分(在绑定crm的情况下 必须强行打积分查询接口去验证)。
    private function check_order_used_point($post,$aCart,&$point_msg){
        //这里根据积分使用方式获取方式 1：只用于兑换  2：只用于抵扣
        $site_point_usage = app::get('b2c')->getConf('site.point_usage');
        $using_point = 0;
        if($site_point_usage == 1){
            //用于兑换
            $using_point = intval($aCart["subtotal_consume_score"]);
        }
        if($site_point_usage == 2){
            //用于抵扣
            $using_point = intval($post["point"]["score"]);
        }
        //没有使用到积分
        if($using_point == 0){
            $point_msg = "未使用积分";
        ...
        //在使用积分的情况下 不开启积分功能或者没有$site_point_usage
        $get_policy_method = $this->app->getConf('site.get_policy.method');
        if($using_point>0 && (!$site_point_usage || $get_policy_method == 1)){
            $point_msg = "请联系管理员查看积分使用的设置";
            return false;
        }
        //在绑定crm或者没有绑定crm的两种情况下 都获取实际可用积分值
        $obj_point_common = kernel::single('pointprofessional_point_common');
        $return_arr = $obj_point_common->check_used_point_get($post["member_id"]);
        //crm查询接口未成功返回
        if (!$return_arr["rs"]){
            $point_msg = "获取实际可用积分值失败";
            return false;
        }
        $real_point = $return_arr["real_point"];
        if($using_point>$real_point){
            $point_msg = "实际可用积分不足";
            return false;
        }
        //都通过返回true
