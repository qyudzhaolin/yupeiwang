...
//        exit;
        ...
        //2017年11月23日19:06:47 将全部货品列表传至前台，让所有按钮可以点击
        $productList = $productsModel->getList('product_id,spec_desc', array('goods_id' => $goodsId));
        ...
        /** 商品规格重新排序 **/
        foreach ($productBasic['spec']['goods'] as $key => $val) {
            $keys[$key] = $data_new[$key] = array();
            foreach ($val as $k => $v) {
                if (empty($v['p_order'])) {
                    $keys[$key][$k] = $v['private_spec_value_id'];
                } else {
                    $keys[$key][$k] = $v['p_order'];
                }
            }
            asort($keys[$key]);
            reset($keys[$key]);
            foreach ($keys[$key] as $k => $v) {
                $data_new[$key][$k] = $val[$k];
            }
        }
        $productBasic['spec']['goods'] = $data_new;
        /** 商品规格重新排序 **/
        /** 去重 **/
        $res = array();
        $newres = array();
        foreach ($data_new as $ks => $vals) {
            foreach ($vals as $values) {
                //查看有没有重复项
                if (isset($res[$ks][$values['spec_value_id']])) {
                    //有：销毁
                    unset($values);
                } else {
                    $res[$ks][$values['spec_value_id']] = $values;
                    $newres[$ks][$values['private_spec_value_id']]=$values;
                }
            }
        }
        $productBasic['spec']['goods'] = $newres;
        /** 去重结束 **/

        //2017年11月24日22:41:40 数据去重
//        $spec_goods = $productBasic['spec']['goods'];
//        foreach ($spec_goods as $pkey => $pitem){
//            $spec_arr = [];
//            foreach ($pitem as $key => $item){
//                if(isset($item['product_id']) || $key == $productBasic['spec']['product'][$pkey]){
//                    if(isset($spec_arr[$item['spec_value_id']])){
//                        if(!$spec_arr[$item['spec_value_id']]['have_id']){
//                            unset($spec_goods[$pkey][$spec_arr[$item['spec_value_id']]['key']]);
//                        }
//                    }
//                    $spec_arr[$item['spec_value_id']]['key'] = $key;
//                    $spec_arr[$item['spec_value_id']]['have_id'] = true;
//                }
//                else{
//                    if(isset($spec_arr[$item['spec_value_id']])){
//                        unset($spec_goods[$pkey][$key]);
//                    }else{
//                        $spec_arr[$item['spec_value_id']]['key'] = $key;
//                        $spec_arr[$item['spec_value_id']]['have_id'] = false;
//                    }
//                }
//            }
//        }
//        $productBasic['spec']['goods'] = $spec_goods;
        // --- 数据去重结束
        //
             
         $sql = "select * from sdb_preparesell_preparesell as     p,sdb_preparesell_preparesell_goods as pg
              where p.goods_id=".$goodsId."  and p.prepare_id=pg.prepare_id" ;
        $result = $mdl_preparesell->db->select($sql);
        if($result[0]['initial_num']){
             $this->pagedata['initial_num'] =$result[0]['initial_num'];   
        }

        if ($result) {
            $this->pagedata['is_prepare'] = 1;   
        }

        //是否设置预售库存数量限制

        $sql1 = "select * from sdb_preparesell_preparesell as     p,sdb_preparesell_preparesell_goods as pg
              where p.goods_id=".$goodsId."  and p.prepare_id=pg.prepare_id and stock_state=true" ;
        $result1 = $mdl_preparesell->db->select($sql1);
     
        if($result1[0]['stock_num']){
             $this->pagedata['stock_num'] =$result1[0]['stock_num'];   
            
        }
        //客户购买上限和下限
        $sql2 = "select * from sdb_preparesell_preparesell as     p,sdb_preparesell_preparesell_goods as pg
              where p.goods_id=".$goodsId."  and p.prepare_id=pg.prepare_id and custmer_state=true" ;
        $result2 = $mdl_preparesell->db->select($sql2);
      
        if($result2){
             $this->pagedata['custmer_upperlimit'] =$result2[0]['custmer_upperlimit'];  
             $this->pagedata['custmer_lowerlimit'] =$result2[0]['custmer_lowerlimit']; 
        ...
        //配送范围检测
        // $this->pagedata['shiparea'] = false;
        // $mdl_shiparea = $this->app->model('shiparea');
        // $shipareaCheckRes = [];
        // $mdl_shiparea->makeProductfilterByShiparea($shipareaCheckRes,false);
        // if (in_array($productId, $shipareaCheckRes['product_id'])) {
        //     $this->pagedata['shiparea'] = true;
        // }
        $this->pagedata['shiparea'] = true;
     ...
     */
    private
    function _get_servicelist_by($servicelist)
    {
     ...
     * */
    private
    function get_body_async_url($aGoods)
    {
        $list = array();
...
//咨询
    ...
    {
        $comments = kernel::single("b2c_goods_description_comments")->show($gid, 'ask', $limit);
...
//获取相关商品
        ...
        if (!cachemgr::get('ajax_product_price' . $product_id, $price)) {
            ...
            if ($prepare) {
                ...
                if (!empty($pre) && $pre['promotion_type'] == 'prepare') {
                    ...
                    // $price['price'] = $this->objMath->formatNumber($pre['preparesell_price'],2);
                    // $price['sales_price'] = $this->objMath->formatNumber($pre['sales_price'],2);
                    
                    //预售预付款价格#TODO#
                    $fundModel = app::get('preparesell')->model('preparesell_fund');
                    $fundWheres = ['prepare_id'=>$pre['prepare_id'],'product_id'=>$product_id,'fund_name'=>'y'];
                    $price['prepare_price'] = $fundModel->getFundPayment('*',$fundWheres);
                }
            }
    ...
    function _get_product_store($product_id)
    {
        ...
        $mdl_preparesell = app::get('preparesell')->model('preparesell');
        $fields = 'goods_id,store,freez,storehouse_id,owner_code,sku_code,is_yp_store';
        $product = app::get('b2c')->model('products')->getList($fields, array('product_id' => $product_id));
        if ($product) {
            $goodsdata = app::get('b2c')->model('goods')->getList('goods_id,nostore_sell,store,store_prompt', array('goods_id' => $product[0]['goods_id']));
            $sql = "select * from sdb_preparesell_preparesell as     p,sdb_preparesell_preparesell_goods as pg
                    where p.goods_id=".$product[0]['goods_id']."  and p.prepare_id=pg.prepare_id  and pg.product_id={$product_id}" ;
            $result = $mdl_preparesell->db->select($sql);
            if ($goodsdata && ($goodsdata[0]['nostore_sell'] || $goodsdata[0]['store'] === null)) {
                $store['store'] = 999999;//暂时表示库存无限大
                if ($result) {
                    $store['store'] =$result[0]['initial_num'];//暂时表示库存无限大
                }
            } else {
            ...
            switch ($show_storage) {
                ...
                case '2';//显示库存数量提示
                    ...
                    if ($result) {
                        $store['title'] =$result[0]['initial_num'];
                    }
                    break;
                case '3';//启用库存优化方案
            ...
            //如果此商品是宇培管库,获取宇培管库商品库存
            // $aproduct = $product[0];
            // if ($aproduct['is_yp_store'] == '1') {
            //     $ypStore = $this->getYpStore($aproduct['storehouse_id'], $aproduct['owner_code'], $aproduct['sku_code']);
            //     $store['store'] = $ypStore['store'];
            //     $store['unit'] = $ypStore['unit'];
            // }
    ...
    //获取云平台商品-返回array//由于采取刷新库存措施，所以没用到-作废
    function getYpStore($storehouse_id='',$owner_code='',$sku_code=''){
        $datas = [];
        $params = [];
        //仓库id
        if (!empty($storehouse_id)) {
            $params['projectIdList'] = $storehouse_id;
        }

        //货主编码
        if (!empty($owner_code)) {
            $params['ownerCodeList'] = $owner_code;
        }

        //商品编码
        if (!empty($sku_code)) {
            $params['skuCodeList'] = $sku_code;
        }
        //商品库存
        $datas = $this->apiget(getConfig('goodsStore'),$params);
        if ($datas['error'] == 0) {
            $res = $datas['data']['rows'][0];
            return ['store'=>$res['qtyUomForYupeiwang'],'unit'=>$res['shipUomName']];
        }
        return $datas;
    }
    ...
    function ajax_product_basic($product_id)
    {
        ...
        if (!cachemgr::get('ajax_product_basic' . $product_id, $basic_html)) {
            ...
            $productsModel = $this->app->model('products');
            $productList = $productsModel->getList('product_id,spec_desc', array('goods_id' => $goodsId));
//        var_dump($productList[0]['spec_desc']['spec_private_value_id']);
            $this->pagedata['productList'] = $productList;
...
//            $productBasic['spec']['goods'] = $spec_goods;
            ...
            // if (in_array($product_id, $shipareaCheckRes['product_id'])) {
            //     $this->pagedata['shiparea'] = true;
            // }
     ...
     * */
    private
    function _get_goods_params($params)
    {
        if (!empty($params)) {
    ...
    function _get_product_price($productId, $aGoods, $member_lv = null)
    {
        ...
        //销售标准价
        $productMemberPrice['sales_price'] = $objCurrency->changer_odr($aGoods['product']['sales_price'], $_COOKIE['S']['CUR'], true, false, $money_format['decimals'], $money_format['fonttend_decimal_type']);
    ...
    function _get_product_basic($productId, $aGoods, $siteMember)
    {
        ...
        $productBasic['sales_price'] = $aGoods['product']['sales_price'];//销售标准价
        $productBasic['commodity_property'] = $aGoods['commodity_property'];//商品属性
        $productBasic['weight'] = $aGoods['product']['weight'];//重量
        $productBasic['store'] = $aGoods['product']['store'];//库存
        $productBasic['is_yp_store'] = $aGoods['product']['is_yp_store'];//是否宇培管库
        ...
        if ($prepare) {
            ...
            if (!empty($pre)) {
                ...
                // $productBasic['price'] = $pre['preparesell_price'];
                // $productBasic['sales_price'] = $this->objMath->formatNumber($pre['sales_price'],2);
                // $productBasic['price'] = $this->objMath->formatNumber($productBasic['price'],2);

                //获取预售款项信息#TODO#
                $fundModel = app::get('preparesell')->model('preparesell_fund');
                $fundWheres = ['prepare_id'=>$pre['prepare_id']];
                $prepareFund = $fundModel->getList('*',$fundWheres);
                $fundModel->translateToPrice($prepareFund, $pre['prepare_id'], $productId);
                $productBasic['prepareFund'] = $prepareFund;
                $productBasic['prepareFundYtimes'] = $fundModel->getFundTimes($prepareFund);//获取预付款时间
            }
        }
        ...
        if ($object_price && $object_price->ifSpecial($productBasic['product_id'])) {
            foreach ($productBasic['promotion'] as $k => $v) {
                if ($k != "order") {
                    unset($productBasic['promotion'][$k]);
                }
            }
        }

        //原产地
        $mdl_gprovenance = $this->app->model('gprovenance');
        $provenance_id = $aGoods['provenance_id'];
        $gprovenanceRow = $mdl_gprovenance->getRow('provenance_id,gsource_area area',['provenance_id'=>$provenance_id]);
        $productBasic['area'] = $gprovenanceRow ? $gprovenanceRow['area'] : '';
...
//获取货品规格数据
    function _get_goods_spec($aGoods)
    {
        ...
        $goods_new = array();
        $key_i = 0;
        foreach ($goodsSpec['specification']['spec_name'] as $key => $val) {
            $key_array[] = $key;
            $key_new[] = $key_i;
            $key_i++;
        }
        foreach ($key_new as $value) {
            $spec_product[$value] = $goodsSpec['product'][$key_array[$value]];
            $goods_new[$value] = $goodsSpec['goods'][$key_array[$value]];
            $spec_name_new[$value] = $goodsSpec['specification']['spec_name'][$key_array[$value]];
            $spec_type_new[$value] = $goodsSpec['specification']['spec_type'][$key_array[$value]];
        }
        $goodsSpec['product'] = $spec_product;
        $goodsSpec['goods'] = $goods_new;
        $goodsSpec['specification']['spec_name'] = $spec_name_new;
        $goodsSpec['specification']['spec_type'] = $spec_type_new;
        $goodsSpec['key'] = $key_array;
        // ee($goodsSpec);
        return $goodsSpec;
    }
                ...
                if ($aAdj = $this->app->model('products')->getList($cols, $gfilter, 0, -1)) {
                    foreach ($aAdj as $k => $aAdj_row) {
                        if ($rows['set_price'] == 'minus') {
                            $adjprice = $aAdj_row['price'] - $rows['price'];
