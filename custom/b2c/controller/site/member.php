...
class b2c_ctl_site_member extends b2c_frontpage{
        ...
        $arr_bases = array(
            array(
                ...
                'items'=>array(
                    ...
                    array('label'=>app::get('b2c')->_('我的预售'),'app'=>'b2c','ctl'=>'site_member','link'=>'prepare'),
                    array('label'=>app::get('b2c')->_('我的合约'),'app'=>'b2c','ctl'=>'site_member','link'=>'myContract'),
            ...
            array(
                ...
                'mid'=>4,
                'items'=>array(
                     // array('label'=>app::get('b2c')->_('企业实名认证'),'app'=>'b2c','ctl'=>'site_member','link'=>'company'),
                    ...
                    array('label'=>app::get('b2c')->_('我的银行卡'),'app'=>'b2c','ctl'=>'site_member','link'=>'bankcard'),
                ),
            ),
        );
    ...
    public function index() {
        ...
        //不包括预售#TODO#
        $aData = $oRder->fetchByMember($this->app->member_id,$nPage=1,array('promotion_type'=>'normal'),5); //--141sql优化点
        //获取预售信息主要是时间219-231
    ...
    * */
    function bankcard(){
        ...
        $objMem = $this->app->model('member_bankcard');
        $this->pagedata['browser'] = $this->get_browser();
        $cards = $objMem->getList('*',array('member_id'=>$this->app->member_id,'effective'=>'1'));
        ...
        $this->pagedata['bankcard']=$cards;

        $this->pagedata['num'] = count($cards);
        $this->output();
    ...
    public function orders($pay_status='all', $nPage=1)
    {
        ...
        $order_status = [];
        $order_status['promotion_type|noequal'] = 'bail';
        if ($pay_status == 'all')
        {
            ...
            foreach ($aData['data'] as $key => $value) {
                if($value['promotion_type']=='prepare')
                {
                    ...
                    if (!empty($prepare)) {
                        ...
                        $fundWhere=['prepare_id'=>$prepare['prepare_id'],'order_id'=>$value['order_id']];
                        $fundRows=$orderFundModel->getList('*',$fundWhere,0,-1,'begin_time asc');
                        $aData['data'][$key]['prepare_fund']=$fundRows;
                        $this->pagedata['nowtime'] = time();
                        
                        //预售订单款项付款状态
                        $prepareOrderModel = app::get('preparesell')->model('prepare_order');
                        $aData['data'][$key]['prepareStatus'] = $prepareOrderModel->fundStatus($value['order_id']);
                        // if ($value['order_id'] == '180608130112510') {
                        //     ee($aData['data'][$key]['prepareStatus'],0);
                        // }
                        // ee($aData['data'][$key]['prepareStatus'],0);
                    }
        ...
        foreach ($aData['data'] as $key => $value) {
            ...
            if($value['promotion_type']=='contract'){
        ...
        $un_pay_orders = $oRder->count(array('member_id' => $this->member['member_id'],'pay_status' => 0,'status'=>'active','promotion_type' => 'normal'));
        // ee($aData['data']);
        $this->pagedata['un_pay_orders'] = $un_pay_orders;
        $this->pagedata['orders'] = $aData['data'];
    ...
    public function orderdetail($order_id=0, $archive=false)
    {
        ...
        if($sdf_order['promotion_type']=='prepare'){
            ...
            //预售支付流水
            $orderFundModel = app::get('preparesell')->model('order_fund');
            $fundPayWater=$orderFundModel->getFundPayWater($sdf_order['order_id']);
            $this->pagedata['fundPayWater'] = $fundPayWater;
        }
        ...
        if ( $arr_order_logs['data']) {
               $data=$arr_order_logs['data'];
            foreach ($data as $key =>$value) {
                if($value['behavior']=='confirm'&&$value['result']=='SUCCESS'){
             $data[$key]['log_text']='订单财务确认成功！';
                }
            }
           $arr_order_logs['data']=$data;
        }
        ...
        //是否是合约订单的检测
        $this->pagedata['buyType']='';
        $mdl_contract_order = app::get('b2c')->model('contract_order');//合约订单
        $contract_order_row = $mdl_contract_order->getRow('*', ['order_id'=>$order_id]);
        if (!empty($contract_order_row)) {
            $this->pagedata['buyType']='contract';
            $this->pagedata['step_type']=$contract_order_row['step_type'];
        }


        //获取物流信息
        $objDelivery = $this->app->model('delivery');
        $this->pagedata['order']['delivery'] = $objDelivery->getRow('*',['order_id'=>$order_id]);
        $this->pagedata['cloudLink']=getConfig('cloudLink');//云平台链接地址
        $this->pagedata['ctime']=time();//当前时间
        // ee($this->pagedata);
        $this->output();
    }
    ...
    function get_msg_num(){
     ...
     //企业实名认证
    function company($nPage=1) {
        $this->get_msg_num();
        $oMsg = kernel::single('b2c_message_msg');
        $row = $oMsg->getList('*',array('to_id' => $this->app->member_id,'has_sent' => 'true','for_comment_id' => 'all','company' => 'true','mem_read_status' => 'false'));
        $this->pagedata['inbox_num'] = count($row)?count($row):0;

        $row = $oMsg->getList('*',array('to_id' => $this->app->member_id,'has_sent' => 'true','for_comment_id' => 'all','company' => 'true'));
        $aData['data'] = $row;
        $aData['total'] = count($row);
        $count = count($row);
        $aPage = $this->get_start($nPage,$count);
        $params['data'] = $oMsg->getList('*',array('to_id' => $this->app->member_id,'has_sent' => 'true','for_comment_id' => 'all','company' =>'true'),$aPage['start'],$this->pagesize);
        $params['page'] = $aPage['maxPage'];
        $this->pagedata['message'] = $params['data'];
        $this->pagedata['total_msg'] = $aData['total'];
        $this->pagination($nPage,$params['page'],'company');
        $this->output();
    }
    //收件箱
    ...
    function view_msg(){
        ...
        if($aMsg[0]&&($aMsg[0]['author_id']!=$this->app->member_id&&$aMsg[0]['to_id']!=$this->app->member_id)){
            ...
            echo app::get('b2c')->_('对不起，您没有权限这条信息！');exit;
        }
    ...
    function save_bankcard(){
        ...
        $param['AccountName'] = $_POST['AccountName'];
        $param['AccountNumber'] = $_POST['AccountNumber'];
        $param['IdentificationType'] = $_POST['IdentificationType'];
        $param['IdentificationNumber'] = $_POST['IdentificationNumber'];
        $param['PhoneNumber'] = $_POST['PhoneNumber'];
        $param['CardType'] = $_POST['CardType'];
        $param['ValidDate'] = $_POST['ValidDate'];
        $param['CVN2'] = $_POST['CVN2'];

        $cpcnquick_obj = kernel::single("ectools_payment_plugin_cpcnquick");
        $res=$cpcnquick_obj->savecard($param);
        if($res['status']=='succ'){
            //绑卡成功
            $save['TxSNBinding'] = $res['TxSNBinding'];
            $save['member_id'] = $this->app->member_id;
            $save['bank'] =$this->bankidToname($res['BankID']);
            $save['AccountName'] =$res['AccountName'];
            $save['AccountNumber'] =$res['AccountNumber'];
            $save['ValidDate'] =$param['ValidDate'];
            $save['CVN2'] =$param['CVN2'];
            $save['effective'] ='0';
            $save['last_modify'] =time();
            $member_bankcard_model = app::get('b2c')->model('member_bankcard');
            $ok = $member_bankcard_model->save($save);


            if ($ok){//短信发送成功返回绑定所需信息
                $res['TxSNBinding']= $save['TxSNBinding'];
                $res['CVN2']= $param['CVN2'];
                $res['ValidDate']= $param['ValidDate'];
     ...
     * */
    function checkcode(){
        $ValidDate=$_POST['ValidDate'] ? $_POST['ValidDate'] : '';
        $CVN2=$_POST['CVN2'] ? $_POST['CVN2'] : '';
        $TxSNBinding=$_POST['TxSNBinding'] ? $_POST['TxSNBinding'] : '';
        $data = array(
            'SMSValidationCode' => $_POST['CodeNumber'][0],
            'TxSNBinding' => $TxSNBinding,
            'ValidDate' => $ValidDate,
            'CVN2' => $CVN2
        );
        $cpcn_obj = kernel::single("ectools_payment_plugin_cpcnquick");
        $res = $cpcn_obj->checkcode($data);
        if ($res['sta']=='succ'){
            if($res['Status']==30){
                $member_bankcard_model = app::get('b2c')->model('member_bankcard');
                $ok=$member_bankcard_model->update(array('effective'=>'1'));
                echo json_encode($res);exit();
            }else{
                $res['sta']='error';
                echo json_encode($res);exit();
...
//             $membersData = $this->get_current_member();
    ...
    */
    ...
    public  function prepare($id=null){
        $this->path[] = array('title'=>app::get('b2c')->_('会员中心'),'link'=>$this->gen_url(array('app'=>'b2c', 'ctl'=>'site_member', 'act'=>'index','full'=>1)));
        $this->path[] = array('title'=>app::get('b2c')->_('预售'),'link'=>'#');
        $GLOBALS['runtime']['path'] = $this->path;
        $order = $this->app->model('orders');
        ...
        foreach ($aData['data'] as $key => $value) {
            ...
            if($value['promotion_type']=='contract'){
             ...
             if($value['promotion_type']=='normal'){
                 unset($aData['data'][$key]);
            }
             if($id==1){
                 if($value['pay_status']!='0'||$value['status']!='active'){
                  unset($aData['data'][$key]);
         ...
         //未付款数
        $oRder = $this->app->model('orders');//--11sql
        $un_pay_orders = $oRder->count(array('member_id' => $this->member['member_id'],'pay_status' => 0,'status'=>'active','promotion_type' => 'prepare'));
        // ee($aData['data']);
        $this->pagedata['un_pay_orders'] = $un_pay_orders;
        $this->pagedata['orders'] = $aData['data'];
        $arr_args = array($pay_status);
    ...
    //我的合约
    public function myContract($nopay=null){
        $this->path[] = array('title'=>app::get('b2c')->_('会员中心'),'link'=>$this->gen_url(array('app'=>'b2c', 'ctl'=>'site_member', 'act'=>'index','full'=>1)));
        $this->path[] = array('title'=>app::get('b2c')->_('我的合约'),'link'=>'#');
        $GLOBALS['runtime']['path'] = $this->path;
        //获取会员登录信息
        ...
        $orderBy = " GROUP BY  c.contract_id  ORDER BY  c.mtime  desc";
        $whereStr='1 and c.member_id='.$arrMember['member_id'].' ';
           //SQL
                   $join = '';
                   $fields = 'c.contract_id,c.contract_no,c.state,c.amount,c.member_id,c.begin_time,c.end_time,c.content,c.accounts,c.pay_status,c.ship_status,c.mtime,c.min_amount,c.min_num,ca.end_time as time, ca.fee_ids';
                   $join .= ' LEFT JOIN sdb_b2c_contract_account_step ca ON c.contract_id=ca.contract_id AND ca.fee_ids = 8 AND ca.state = "on"';            
                   $sql ='select ' . $fields 
                        . ' from' 
                        . ' `sdb_b2c_contract` c ' 
                        . $join
        ...
        if($results){
            foreach ($results as $key => $value) {
                //出库信息
                $sql ="select o.payment,o.pay_status, co.step_type,o.ship_status,co.order_id,co.contract_id,o.final_amount,
                     o.createtime,co.member_id,o.received_status  from 
                     sdb_b2c_orders as o,sdb_b2c_contract_order as co WHERE co.order_id=o.order_id 
                     AND co.member_id=".$value['member_id']." and co.contract_id=".$value['contract_id']." and (o.status='active' or o.status='finish')";
                $delivery_order= $contractModel->db->select($sql);
                if($delivery_order){
                    foreach ($delivery_order as $k =>$v) {
                        //支付方式
                       $opayment = app::get('ectools')->model('payments');
                       $payment = $opayment->dump(array('pay_app_id'=>$v['payment']), 'pay_name', $subsdf=null);  
                       $delivery_order[$k]['payment']= $payment['pay_name'];
                       if($v['pay_status']==0){
                        $results[$key]['order_id']=$v['order_id'];
                        $count=$count+1;
                       }
                         //订单状态
                           $tmpArr1 = array(
                        ...
                        $delivery_order[$k]['pay_status']= $tmpArr1[$v['pay_status']];
                         //发货状态
                           $tmpArr2 = array(
                            0 => app::get('b2c')->_('未发货'),
                            1 => app::get('b2c')->_('已发货'),
                        ...
                        $delivery_order[$k]['ship_status']= $tmpArr2[$v['ship_status']];
                        $delivery_order[$k]['createtime']=date('Y.m.d',$v['createtime']);    
                    }
                ...
                //合约出库单
                 if($value['fee_ids']==8){
                     $results[$key]['delivery_order']=$delivery_order;
                 }
    ...
    public  function contract_record(){
        ...
        if($type=1){
            if($order_id){
                $result = $objOrder->db->exec("UPDATE sdb_b2c_orders set promotion_type='contract' where order_id =".$order_id." "); 
            }
        }
    }
    //查看合约
    public function viewContract($arg0){
        $this->path[] = array('title'=>app::get('b2c')->_('会员中心'),'link'=>$this->gen_url(array('app'=>'b2c', 'ctl'=>'site_member', 'act'=>'index','full'=>1)));
        $this->path[] = array('title'=>app::get('b2c')->_('我的合约'),'link'=>'#');
        $mdl_contract = app::get('b2c')->model('contract');
       //获取会员登录信息
         $arrMember = kernel::single('b2c_user_object')->get_members_data(array('members'=>'member_id,cur',));
         $arrMember = $arrMember['members'];
         $arg1=$arrMember['member_id'];
       //合约基本信息
       $result=$mdl_contract->getContract($arg0,$arg1);
        if($result){
        $mdl_contract= app::get('b2c')->model('contract');
        //付款状态;
        $contract_id=$result[0]['contract_id'];
        $pay_status=$mdl_contract->getContractPayStatus($contract_id);
        $result[0]['pay_status'] = $pay_status['message'];    
        $this->pagedata['contract'] = $result; 

          }
       //商品明细
        $goods=$mdl_contract->getGoods($arg0,$arg1);

        if($goods){
        $this->pagedata['goods'] = $goods; 
          }
       //出库流水
        $goodsout=$mdl_contract->getGoodsout($arg0,$arg1);
        $this->objMath = kernel::single("ectools_math");
        if($goodsout){
           foreach ($goodsout as $key => $value) {
                  $a=$this->objMath->formatNumber($value['price'],2);
                  if($a!='0.00'){
                    $goodsout[$key]['price']=$a;   # code...
                  }
                
           }
          $this->pagedata['goodsout'] = $goodsout;    
        }
       //支付流水
        $contractpayment=$mdl_contract->getPayment($arg0,$arg1);
        if($contractpayment){
          $this->pagedata['contractpayment'] = $contractpayment;
        }
        //物流状态
        $ship_status=$mdl_contract->contract_ships_status($arg0);
        if($ship_status){
     ...
     function cancel_delivery(){
        ...
        $contract_goods_out = $this->app->model('contract_goods_out');
            //作废订单
            if($_POST['order_id']){
                $status="'dead'";
                $result = $objOrder->db->exec("UPDATE sdb_b2c_orders set status=".$status." where order_id =".$_POST['order_id']); 
            //库存变化
            //查询合约产品id
                if($_POST['contract_id']){
                    $sql1 ="select product_id  from sdb_b2c_contract_goods  where contract_id=".$_POST['contract_id'];
                    $product_id=$contract_goods->db->select($sql1);
                    if($product_id){
                         //原先出库录数
                        foreach ($product_id as $key => $value) {
                            # code...
                             $sql2 ="select num from  sdb_b2c_contract_goods_out  where   product_id=".$value['product_id']."  and  contract_id=".$_POST['contract_id']." and order_id=".$_POST['order_id']."
                             ";
                        $amount= $contract_goods_out->db->select($sql2);
                        if($amount){
                             //原先库存加记录数
                             $amountadd= $contract_goods->db->exec("UPDATE sdb_b2c_contract_goods set store_left=store_left+".$amount[0]['num']." where product_id=".$value['product_id']." and contract_id=".$_POST['contract_id']);  
    ...
    function confirm_receipt(){
        ...
        $contract_goods_out = $this->app->model('contract_goods_out');
            ...
            if($_POST['order_id']){
                ...
                $pay_status="'1'";
                $ship_status="'1'";
                $result = $objOrder->db->exec("UPDATE sdb_b2c_orders set received_status=".$received_status." where order_id =".$_POST['order_id']." and pay_status=".$pay_status." and ship_status=".$ship_status.""); 
    ...
    //查看合约出库单
    function viewContractOutbound($order_id){
         // $order_id=180730145280680;
        $this->path[] = array('title'=>app::get('b2c')->_('会员中心'),'link'=>$this->gen_url(array('app'=>'b2c', 'ctl'=>'site_member', 'act'=>'index','full'=>1)));
        $this->path[] = array('title'=>app::get('b2c')->_('我的合约'),'link'=>'#');
         $GLOBALS['runtime']['path'] = $this->path;
       if (!isset($order_id) || !$order_id)
