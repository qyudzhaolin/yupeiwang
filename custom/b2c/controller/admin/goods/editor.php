...
/*{{{*/
        ...
        //仓库列表
        $storehouseData = $this->apiget(getConfig('getEbProjectList'));
        if ($storehouseData['error'] == 0) {
            $nullItem = [0=>['ebpjId'=>'0','ebpjWhName'=>'----无----']];
            $this->pagedata['storehouseData'] = array_merge($nullItem, $storehouseData['data']['rows']);
        }
        // ee($this->pagedata['storehouseData']);



        //货主列表
        $ownerData = $this->apiget(getConfig('ownerList'));
        // ee($ownerData);
        if ($storehouseData['error'] == 0) {
            // $nullItem = [0=>['id'=>'','name'=>'----无----']];
            // $this->pagedata['ownerData'] = array_merge($nullItem, $ownerData['data']['rows']);


            //组装id=>name对应数组
            $tempOwnerDatas = $ownerData['data']['rows'];
            $ownerDatas = [];
            foreach ($tempOwnerDatas as $ownerData) {
                $ownerDatas[$ownerData['id']] = $ownerData['name'];
            }
            $this->pagedata['ownerDatas'] = $ownerDatas;
        }

        //原产地
        $ogprovenance = $this->app->model('gprovenance');
        $gprovenanceList = $ogprovenance->getList('provenance_id,gsource_area','',0,-1);
        $nullItem = [0=>['provenance_id'=>'0','gsource_area'=>'----无----']];
        $this->pagedata['gprovenanceList'] = array_merge($nullItem, $gprovenanceList);
        // ee($this->pagedata['gprovenanceList']);

        //供应商
        $osupplier = $this->app->model('supplier');
        $supplierList = $osupplier->getList('supplier_id,shortname','',0,-1);
        $nullItem = [0=>['supplier_id'=>'0','shortname'=>'----无----']];
        $this->pagedata['supplierList'] = array_merge($nullItem, $supplierList);

        //配送范围
        $this->pagedata['shiparea'] = [];
        $shipareas = $this->app->model('shiparea')->getShiparea(['goods_id'=>$this->goods_id]);
        if($shipareas) {
            $tmpAreas=[];
            foreach ($shipareas as $shiparea) {
                $tmpAreas[$shiparea['region_id']] = $shiparea['name'];
            }
            $this->pagedata['shiparea'] = $tmpAreas;
        }
...
/*}}}*/
    ...
    function toAdd(){
        ...
        if($prepare&&method_exists($prepare,'is_update'))
        {
            ...
            //当前商品有预售规则 并且 尾款结束时间大于当前 商品不能做信息修改
            if(!empty($pre) && $pre["end_time_final"] >= time())
            {
        ...
        //保存配送范围
        $shipareaDatas = [];
        $shipareaDatas['shiparea'] = isset($_POST['goods']['shiparea']) ? $_POST['goods']['shiparea'] : '';
        $shipareaDatas['goods_id'] = isset($goods['goods_id']) ? $goods['goods_id'] : '';
        $this->app->model('shiparea')->saveShiparea($shipareaDatas);
...
//        $this->pagedata['spec_default_pic'] = $this->app->getConf('spec.default.pic');
    ...
    function get_regions(){
        ...
        $this->pagedata['shiparea'] = explode(',', $shiparea);
        $this->pagedata['datas'] = $regionsTree;
        exit($this->fetch('admin/goods/detail/regions.html'));
    }
}
