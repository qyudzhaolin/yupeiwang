...
class b2c_ctl_admin_goods extends desktop_controller{
    ...
    function index(){
        ...
        $group[] = array('label'=>app::get('b2c')->_('商品配送范围'),'icon'=>'download.gif','submit'=>'index.php?app=b2c&ctl=admin_goods&act=singleBatchEdit&p[0]=shiparea','target'=>'dialog');


        $group[] = array('label'=>app::get('b2c')->_('商品原产地'),'icon'=>'download.gif','submit'=>'index.php?app=b2c&ctl=admin_goods&act=singleBatchEdit&p[0]=gprovenance','target'=>'dialog');

        $group[] = array('label'=>app::get('b2c')->_('商品供应商'),'icon'=>'download.gif','submit'=>'index.php?app=b2c&ctl=admin_goods&act=singleBatchEdit&p[0]=supplier','target'=>'dialog');
        $group[] = array('label'=>app::get('b2c')->_('销售方式'),'icon'=>'download.gif','submit'=>'index.php?app=b2c&ctl=admin_goods&act=singleBatchEdit&p[0]=salesmode','target'=>'dialog');
...
//         );
   ...
   function _views(){
        ...
        //显示当前tab文字旁的统计数字
        foreach($show_menu as $var_k=>&$var_v){
            if ($var_k == intval($_GET['view'])){
                $var_v["newcount"] = true;
            }
        }
        unset($var_v);
    ...
    function singleBatchEdit($editType=''){
        ...
        switch( $editType ){
          ...
          case 'gprovenance':
                ...
                $oBrand = $this->app->model('gprovenance');
                $brandMap  = $oBrand->brand2json(1);
                $brandList = array();
                foreach( $brandMap as $v )
                    $brandList[$v['provenance_id']] = $v['gsource_area'];
                $this->pagedata['brand'] =  $brandList;
                break;
           ...
           case 'supplier':
                ...
                $brandList = array();
                foreach( $brandMap as $v )
                    $brandList[$v['supplier_id']] = $v['shortname'];
                $this->pagedata['brand'] =  $brandList;
                break;
           case 'salesmode':
                $this->pagedata['brand'] = array(
                1 => '对内对外',
                2 => '仅对内'
                );
                break;
    ...
    function saveBatchEdit(){
        ...
        {
            $pre = $prepare->is_update($filter);
            if(!empty($pre))
            {
                //$this->begin('index.php?app=b2c&ctl=admin_goods&act=index');
                $this->end(false, app::get('b2c')->_('选择的商品中含有预售商品'));
            }
        }
...
//        }
        ...
        switch( $_POST['updateAct'] ){
            ...
            case 'brand':
             ...
             case 'gprovenance':
                ...
                $oBrand = $this->app->model('gprovenance');
                $aBrand = $oBrand->dump(array('provenance_id'=>$_POST['set']['brand']),'gsource_area');
                $oPro->batchUpdateArray( $filter['goods_id'] , 'sdb_b2c_goods', array('provenance_id'), array( intval($_POST['set']['brand']), $aBrand['gsource_area'] ) );
                break;

              case 'supplier':
                $oBrand = $this->app->model('supplier');
                $aBrand = $oBrand->dump(array('supplier_id'=>$_POST['set']['brand']),'shortname');
                $oPro->batchUpdateArray( $filter['goods_id'] , 'sdb_b2c_goods', array('supplier_id'), array( intval($_POST['set']['brand']), $aBrand['shortname'] ) );
                break;  
               case 'salesmode':
                 $oPro->batchUpdateInt( $filter['goods_id'], 'sales_mode', intval($_POST['set']['brand']) );
                break;  
            case 'shiparea':
                //批量编辑配送范围
                $oShiparea = $this->app->model('shiparea');
                $oShiparea->batchUpdateArray($filter['goods_id'], $_POST['goods']['shiparea']);
              break;   
    ...
    //获取云平台商品-返回array
    function apiGetCloudGoods($ownerCode='',$projectId='',$skuName=''){
        $datas = [];
        $params = [];
        //货主编码
        if (!empty($ownerCode)) {
        ...
        //商品列表
        $datas = $this->apiget(getConfig('goodsList'),$params);
        if ($datas['error'] == 0) {
            return $datas['data'];
        }
        return $datas;
    ...
    //获取云平台商品
    function getCloudGoods(){
        $ownerCode = trim($_GET['ownerCode']);
        $projectId = trim($_GET['projectId']);
        $skuName   = trim($_GET['skuName']);
        $sku_code   = trim($_GET['sku_code']);
        if (empty($skuName)) {
            $skuName = '';
        }
        //商品列表
        $datas = $this->apiGetCloudGoods($ownerCode,$projectId,$skuName);
        // ee($datas);
        $this->pagedata['datas'] = $datas;
        $this->pagedata['keyword'] = $skuName;
        $this->pagedata['sku_code'] = $sku_code;
        exit($this->fetch('admin/goods/cloudGoodsSelect.html'));
    }


    //获取云平台商品-库存
    function getCloudGoodStore(){
        $storehouse_id = trim($_GET['storehouse_id']);
        $owner_code = trim($_GET['owner_code']);
        $sku_code   = trim($_GET['sku_code']);

        if (empty($sku_code)) {
            $this->apiReturn(['error'=>1,'message'=>'没有提供sku_code，请选择商品！']);
        }

        $paramsDatas = [];
        $link = '^_^';
        $paramsDatas = $storehouse_id . $link . $owner_code . $link . $sku_code;
        $Tdatas = $this->apiget(getConfig('syncStore'),['fullSkuList'=>$paramsDatas,'pageSize'=>99999999]);
        if ($Tdatas['error']) {
            $this->apiReturn(['error'=>1,'message'=>'获取WMS商品库存接口出错！']);
        }

        if (empty($Tdatas['data']['rows'][0])) {
            $this->apiReturn(['error'=>1,'message'=>'WMS接口调用成功，但并没有提供商品信息！']);
        }

        $data = $Tdatas['data']['rows'][0];
        // ee($datas);
        $this->apiReturn(['error'=>0,'data'=>$data]);
    }


    //获取云平台货主
    function getCloudOwners(){
        $wheres = [];
        $keyword= trim($_GET['keyword']);
        $ownerCode = trim($_GET['ownerCode']);
        $wheres['ownerName'] = $keyword;
        //货主列表
        $datas = $this->apiget(getConfig('ownerList'),$wheres);

        // ee($datas);
        $this->pagedata['datas'] = $datas;
        $this->pagedata['keyword'] = $keyword;
        $this->pagedata['ownerCode'] = $ownerCode;
        exit($this->fetch('admin/goods/cloudOwnersSelect.html'));
    }
