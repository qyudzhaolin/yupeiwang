...
class b2c_ctl_admin_order extends desktop_controller{
    ...
    public function index(){
        ...
        $params = [
            ...
            // 'base_filter' =>array('order_id' => '181119113149787'),
            'use_view_tab'=>true,
        ];

        $this->finder('b2c_mdl_orders',$params);
    }
    ...
    public function _views(){
        ...
        $sub_menu = array(
            0=>array('label'=>app::get('b2c')->_('待发货'),'optional'=>false,'filter'=>array('ship_status'=>array('0','2'),'pay_status'=>array('1','2','3'),'status'=>'active','disabled'=>'false','finance_status'=>array('1'))),
            ...
            19=>array('label'=>app::get('b2c')->_('待财务确认'),'optional'=>false,'filter'=>array('pay_status'=>array('1'),'status'=>'active','disabled'=>'false','finance_status'=>array('0'))),
            ...
            18=>array('label'=>app::get('b2c')->_('合约订单'),'optional'=>false,'filter'=>['disabled'=>'false','promotion_type'=>'contract']),
            22=>array('label'=>app::get('b2c')->_('保证金'),'optional'=>false,'filter'=>['disabled'=>'false','promotion_type'=>'bail']),
        );
        //新留言订单
        ...
        #网点限制#
        if (!empty($this->user)) {
            $regionIds = kernel::single('b2c_access')->checkNetlimit($this->user,'string');
            if (!empty($regionIds)) {
                foreach ($show_menu as $kk => $menu) {
                    $show_menu[$kk]['filter']['filter_sql'] = "1 having region_id in({$regionIds}) ";
                }
            }
        }
    ...
    public function create()
    {
        ...
        foreach($_POST['order']['product_id'] as $product_id)
        {
            ...
            $productName = "{$product['name']}({$product['spec_info']})";
            $goodsRow = $obj_goods->getRow('nostore_sell',['goods_id'=>$product['goods_id']]);
            if ($goodsRow['nostore_sell'] != '1' &&  ($product['store']-$product['freez']) < $_POST['goodsnum'][$product_id] )
            {
                $this->splash('error',null, "{$productName}库存不足，请检查！");
            }
    ...
    public function printing($type,$order_id)
    {
        ...
        //添加供应商
        $obj_g = $this->app->model('goods');
        $obj_s = $this->app->model('supplier');
        $order_itemss=$order_items;
        $dataTmp = [];
       
       foreach ($order_itemss as $key => $value) {
           
          $sql = "select s.shortname as name,s.supplier_id as supplier_id from ".$obj_g->table_name(true)." as g,".$obj_s->table_name(true)." as s  where g.supplier_id=s.supplier_id   and  goods_id=".$value['goods_id'];
            $shortname = $obj_g->db->select($sql);
             $order_itemss[$key]['shortname']= $shortname[0]['name'];  
            $order_itemss[$key]['supplier_id']= $shortname[0]['supplier_id']; 
            ...
            $tmp = [
                ...
                'amount'=>$value['cost']*$value['quantity'],
                'weight'=>$value['weight'],
                ...
                'small_pic'=>$value['small_pic'],
                'is_type'=>$value['is_type'],
                'link_url'=>$value['link_url'],
                'shortname'=>$shortname[0]['name'],
                'supplier_id'=> $shortname[0]['supplier_id'],
            ];


            $supplierID = $order_itemss[$key]['supplier_id'];
           if (!isset($dataTmp[$supplierID])) {
               $dataTmp[$supplierID] = $value;
              

           }
           $dataTmp[$supplierID]['suplierGoods'][] = $tmp;
         $dataTmp[$supplierID]['shortname'] = $shortname[0]['name'];
           
       }

   
         foreach ($dataTmp as $key => $value) {
            $list=$value['suplierGoods'];
           $last_names = array_column($list, 'amount','goods_id');
           $aAccount=array_sum($last_names);
            $dataTmp[$key]['sumamount'] = $this->objMath->formatNumber($aAccount,3);
         }
         // ee($dataTmp);
         $intcount=count($dataTmp);
        $this->pagedata['intcount'] = $intcount;
        $this->pagedata['goodsItem'] = $dataTmp;
        ...
        $this->_systmpl = $this->app->model('member_systmpl');
           $this->pagedata['http'] = getConfig('host');
        switch($type)
        {
            case $order->arr_print_type['ORDER_PRINT_CART']:  /*采购单打印*/
                $this->pagedata['printType'] = array("cart");
                $this->pagedata['printContent']['cart'] = true;
                $this->pagedata['memberPoint'] = $memberInfo[0]['point'] ? $memberInfo[0]['point'] : 0;
                $this->pagedata['content_cart'] = $this->_systmpl->fetch('admin/order/print_sheetbuy',$this->pagedata);

                   // var_dump($order_items);
                $this->pagedata['page_title'] = app::get('b2c')->_('采购单打印');
                $this->display('admin/order/print.html');
                break;
                   
            case $order->arr_print_type['ORDER_PRINT_SHEET']:    /*销售单单*/
                $this->pagedata['printContent']['sheet'] = true;
                  $this->pagedata['goodsItem'] = $order_items;
                $this->pagedata['memberPoint'] = $memberInfo[0]['point'] ? $memberInfo[0]['point'] : 0;
                $this->pagedata['content_sheet'] = $this->_systmpl->fetch('admin/order/print_cartsale',$this->pagedata);
                $this->pagedata['page_title'] = app::get('b2c')->_('销售单打印');
                $this->display('admin/order/print.html');
                break;

            case $order->arr_print_type['ORDER_PRINT_MERGE']:    /*签收单打印*/
                $this->pagedata['printType'] = array("cart");
                $this->pagedata['printContent']['cart'] = true;
                $this->pagedata['printContent']['sheet'] = true;
                $this->pagedata['memberPoint'] = $memberInfo[0]['point'] ? $memberInfo[0]['point'] : 0;
                $this->pagedata['content_cart'] = $this->_systmpl->fetch('admin/order/print_sheetsign',$this->pagedata);
                // $this->pagedata['content_sheet'] = $this->_systmpl->fetch('admin/order/print_sheet',$this->pagedata);
                $this->pagedata['page_title'] = app::get('b2c')->_('签收单打印');
                $this->display('admin/order/print.html');
                break;
     ...
     * @return string html
     */
    public function gopay_contract($contract_id)
    {
        if (empty($contract_id))
        {
            $this->splash('error',null,'合约号传递出错！');
        }
        $mdl_contract = app::get('b2c')->model('contract');//合约主表

        $contractOrderRow = $mdl_contract->getWaitPayOrderRowByContractId($contract_id);
        if ($contractOrderRow['error']) {
            $this->splash('error',null,$contractOrderRow['message']);
        }

        //当阶段中包含出库时,提醒  “分批出库支付请在合约出库单页面完成支付”
        if ($contractOrderRow['step_type'] == 'out') {
            $this->splash('error',null,'分批出库支付请在合约出库单页面完成支付');
        }
        $mdl_step = app::get('b2c')->model('contract_account_step');//结算阶段
        $stepRow = $mdl_step->getRow('*',['step_id'=>$contractOrderRow['step_id']]);
        $contractOrderRow['stepRow'] = $stepRow;
        $order_id = $contractOrderRow['order_id'];
        $this->pagedata['contractOrderRow'] = $contractOrderRow;//待付款订单信息
        $this->gopay($order_id);
    }
    ...
    public function gopay($order_id)
    {
        ...
        $this->listAddPayment($this->pagedata['payment']);
        $this->display('admin/order/gopay.html');
    }

    //列表增加其他支付
    public function listAddPayment(&$payment)
    {
        $newPayment = [
            'app_name'=>'账期支付',
            'app_id'=>'period'
    ...
    {
        $sdf = $_POST;
          if( $sdf['payment'] == 'offline' && !is_numeric($sdf['bank_no'])){
            $msg="银行卡的卡号必须是数字格式";
             eval("\$msg = \"$msg\";");
            header('Content-Type:text/jcmd; charset=utf-8');
        ...
        $sdf['bank_name'] = $sdf['bank_name'];
        $sdf['bank_no'] = $sdf['bank_no'];
    ...
    public function gorefund($order_id)
    {
        ...
        //售前(退款申请中 未发货)退款金额不能编辑
        if($aORet["pay_status"] == "6" && $aORet["ship_status"] == "0"){
            $this->pagedata["money_unedit"] = true;
        }
    ...
    public function godelivery($order_id)
    {
        ...
        $logiInfo = $orderBillObj->dump(array('status'=>0,'type'=>1,'order_id'=>$order_id));
        $this->pagedata['corp_id'] = $logiInfo['logi_id'];
        $this->pagedata['logi_no'] = $logiInfo['logi_no'];
    ...
    public function dodelivery()
    {
        ...
        {
            //如果发货的时候有运单号，更新相应的电子面单使用状态
            $orderBillObj = app::get('express')->model('order_bill');
            if($sdf['logi_no']){
                $orderBillObj->update(array('status'=>'1','last_modify'=>time()),array('order_id'=>$_POST['order_id'],'logi_id'=>$sdf['logi_id'],'logi_no'=>$sdf['logi_no'],'type'=>1));
            }
    ...
    /**
     * 生成WMS出库单
     * @params array $sdf 提交数据
     * @params array $orderItems order_items表数据
     * @return array wms接口返回的数组
     */
    public function createSoOder(&$sdf, &$orderItems){
        // return ['error'=>1,'message'=>'xxxx'];
        // ee($sdf,0);
        // ee($orderItems);
        $params = [];
        if (!empty($orderItems)) {
            foreach ($orderItems as $key=>$product) {
                if ($key == 0) {
                    $params[$key] = [
                        "def1"=>$product['order_id'],//订单号
                        "consigneeAddress"=>$sdf['ship_addr'],//收货地址
                        "def11"=>$sdf['money'],//运费
                        "ownerCode"=>$product['owner_code'],//货主编码
                        "consigneeName"=>$sdf['ship_name'],//收货人
                        "projectId"=>$product['storehouse_id'],//关联WMS里面的仓库
                        "soDetailList"=>[],//订单明细
                    ];
                }

                $params[0]['soDetailList'][] = [
                    "skuCode"=>$product['sku_code'],//关联的WMS商品编码
                    "uom"=>$product['unitEn'],//默认发货单位
                    "qtySoEa"=>$product['nums'],//订单数量
                    "price"=>$product['price'],//单价
                ];
            }
        }
        // ee($params);
        // ee(json_encode($params));//APIMARK
        $result = $this->apiget(getConfig('createSoOder'),['orderJson'=>json_encode($params)]);
     ...
     * @return boolean 成功与否
     */
    public function dofinance($order_id)
    {
       $this->begin();

        $obj_order = $this->app->model('orders');
        $arr_order = $obj_order->dump($order_id);
        $arr_order['finance_status']=1;
        if ($obj_order->save($arr_order))
        {
            ...
            $this->end(true, app::get('b2c')->_('订单确认成功！'));
        }
        else
        {   // 更新日志结果
     
          
            $this->end(false, app::get('b2c')->_('订单确认失败！'));
        }
    }
    /**
     * 订单备注添加，修改
