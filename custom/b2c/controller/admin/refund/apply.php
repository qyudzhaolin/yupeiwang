...
class b2c_ctl_admin_refund_apply extends desktop_controller{
    ...
    public function gorefund($refund_apply_bn,$order_id){
        ...
        $paymentNo=$obj_bills->getRow('bill_id',array('rel_id'=>$order_id));
        $aORet['payinfo']['paymentNo']=$paymentNo['bill_id'];


        if ($aORet['member_id']){
            //20180916去掉原路返还退款项
            //$this->pagedata['typeList'] = array('online'=>app::get('b2c')->_("在线支付"), 'offline'=>app::get('b2c')->_("线下支付"),'refund_old_way'=>app::get('b2c')->_("原路返还"));
        ...
        $opayment = app::get('ectools')->model('payment_cfgs');
        $payname=$opayment->getList('*', array('status' => 'true', 'platform'=>array('iscommon','ispc'), 'is_frontend' => true,'app_id'=>$aORet['payinfo']['pay_app_id']));

        $this->pagedata['order']['payinfo']['appname']=$payname[0]['app_name'];
        $this->pagedata['order']['payinfo']['app_class']=$payname[0]['app_class'];
...
//        echo '<pre>';print_r($sdf);exit();
        ///////////////////原路退款start//////////
//
        if($sdf['pay_type'] == 'refund_old_way' && $sdf['money']>0)
        {
            $objclass=kernel::single($sdf['app_class']);
            if(!method_exists( $objclass, 'dorefund' )){
                $this->end(false,'此单不支持资金原路返回，请走线下支付退款！');
            }

            $apiParams = [
                'refund_id' => $sdf['refund_apply_bn'],
                'payment_id' => $sdf['paymentNo'],
                'money' => $sdf['money'],
            ];
            $refund=kernel::single('b2c_refund_refund');
