...
class b2c_ctl_admin_member extends desktop_controller{
	 ...
	 function index(){
				...
				if (!empty($this->user)) {
				    ...
				    if (!empty($regionIds)) {
				        $actions_base['base_filter']['filter_sql'] = "1 having region_id in({$regionIds}) ";
				    }
				}
	 ...
	 function _views(){
		...
		function add_page(){
				...
				$this->pagedata['member_cate'] = $this->member_model->member_cate;//会员分类
