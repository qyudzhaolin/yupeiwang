...
class b2c_ctl_admin_products extends desktop_controller
{
    ...
    function set_spec_index()
    {
        ...
        if ($prepare) {
            $pre = $prepare->get_Prepare_Detail($goods_id);
        }
    ...
    private function _get_goods_info($goods_info)
    {
        if ($goods_info) {
            ...
            $goods['sales_price'] = $goods_info['price']['sales_price']['price'];
    ...
    public function getProducts($gid = 0)
    {
        ...
        $productData = $productMode->getList('product_id,bn,price,sales_price,cost,mktprice,store,freez,store_place,weight,marketable,spec_desc,is_default', array('goods_id' => $gid));
    ...
    public function save_editor()
    {
        ...
        if ($prepare) {
            ...
            if ($pre['promotion_type'] == 'prepare') {
                $msg = app::get('b2c')->_('该货品中含有预售货品，不可修改！');
        ...
        foreach ($selectionSpec['spec'] as $key => $spec_val) {
            $specValueData = $this->app->model('spec_values')->getList('spec_value_id', array('spec_id' => $key), 0, -1);
    ...
    private function _pre_process_products($products, &$spec)
    {
        ...
        foreach ($products as $uid => $row) {
            ...
            $tmpProducts[$uid]['price']['sales_price']['price'] = $row['sales_price'];
...
//            $tmpProducts[$uid]['spec_desc']['p_order'] = $row['spec_desc'];
            ...
            foreach ($row['spec_desc']['spec_private_value_id'] as $specId => $spec_private_value_id) {
                if ($spec['spec'][$specId][$spec_private_value_id]) {
                    ...
                    $tmpSpec['spec'][$specId][$spec_private_value_id]['p_order'] = $spec['spec'][$specId][$spec_private_value_id]['p_order'];
                } else {
                    //多个货品，但是提交过来的规格数据缺失
