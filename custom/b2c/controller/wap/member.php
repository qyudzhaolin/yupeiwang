...
class b2c_ctl_wap_member extends wap_frontpage{
    ...
    public function index() {
        //测试调用接口
        // $params = ["skuCodeList"=>"201810231705","ownerCodeList"=>"KHZ_1540186180309"];
        // $data = $this->apiget(MSURL . 'ypwmsserv/api/cwmsInventory.do',$params);
        // ee($data);
        //面包屑
       ...
       $msgAlert['un_pay_orders_format'] = $msgAlert['un_pay_orders'] > 99 ? '99+' : $msgAlert['un_pay_orders'];//格式化
       $msgAlert['be_received_orders_format'] = $msgAlert['be_received_orders'] > 99 ? '99+' : $msgAlert['be_received_orders'];//格式化
...
#        $this->pagedata['defaultImage'] = $imageDefault['S']['default_image'];
        ...
        $proveRow = $mdl_prove->getRow('*',['member_id'=>$this->app->member_id,'disabled'=>'false']);

        if (!empty($proveRow)) {
            $proveRow['status_name'] = $this->proveStatusArr[$proveRow['status']];
        }

        $this->pagedata['proveRow'] = $proveRow;

       if ($_GET['t']) {
           $this->page('wap/member/index2.html');
       }else{
    ...
    private function msgAlert(){
        ...
        //获取待收货
        $be_received_orders = $oRder->count(array('member_id' => $this->member['member_id'],'status'=>'active','promotion_type' => 'normal','pay_status' => '1','received_status' => '0'));
        $member['be_received_orders'] = $be_received_orders;
         //获取预售订单数
    ...
    public function orders($pay_status='all', $nPage=1)
    {
        ...
        if ($_GET['t']) {
            $this->page('wap/member/orders(news7).html');
        }else{
...
// echo "<pre>";print_r($this->pagedata['order']);exit;
        ...
        $rs_refund_apply = $mdl_b2c_refund_apply->getRow('*', ["order_id"=>$order_id,"disabled"=>'false']);
        $this->pagedata['rs_refund_apply']=$rs_refund_apply;

        //订单取消记录
        $mdl_order_log = $this->app->model('order_log');
        $wheres = ["rel_id"=>$order_id,"behavior"=>'cancel',"bill_type"=>'order',"result"=>'SUCCESS'];
        $cancel_row = $mdl_order_log->getRow('*', $wheres);
        // ee($cancel_row);
        $this->pagedata['cancel_row']=$cancel_row;

        // ee($rs_refund_apply);
        // ee($this->pagedata['order']);
        if ($_GET['t']) {
        $this->page('wap/member/orderdetail(new).html');
        }else{
    ...
    function modify_receiver($addrId=null){
        ...
        if($obj_member->check_addr($addrId,$this->app->member_id)){
            ...
            if ($_GET['t']) {
                $this->page('wap/member/modify_receiver2.html');
            }else{
    ...
    //生成地址格式数据源
    //使用说明：直接把结果复制到cityJson.js变量里面即可
    // function get_regions(){
    //     $shengshi = [];//省市数据源
    //     $xian = [];//县数据源
    //     $regionsModel = app::get('ectools')->model('regions');
    //     $regions = $regionsModel->get_regions_data(1,2,'','order by letter');
    //     $regionsTree = $regionsModel->gettree($regions);

    //     foreach ($regionsTree as $key => $value) {
    //         $shengshi[$key]['id'] = $value['id'];
    //         $shengshi[$key]['name'] = $value['name'];
    //         $shi=[];
    //         foreach ($value['children'] as $k => $v) {
    //             $shi[] = ['id'=>$v['id'],'name'=>$v['name']];
    //         }
    //         $shengshi[$key]['city'] = $shi;
    //     }
    //     // echo json_encode($shengshi,JSON_UNESCAPED_UNICODE);

    //     //获取县级别数据源
    //     $xian = $regionsModel->getList('region_id id,local_name name,p_region_id pid',['region_grade'=>'3']);
    //     //echo json_encode($xian,JSON_UNESCAPED_UNICODE);
    // }
    ...
    */
    ...
    function security($type = ''){
        ...
        $GLOBALS['runtime']['path'] = $this->path;
        $this->pagedata['title'] = '修改密码';
        $this->page('wap/member/modify_password.html');
    }
    /*
     *会员中心 联系我们
     * */
    function call_with($type = ''){
        $member = $this->member;
        $obj_pam_members = app::get('pam')->model('members');
        $this->pagedata['is_nopassword'] = $obj_pam_members->is_nopassword($member['member_id']);
        $this->path[] = array('title'=>app::get('b2c')->_('会员中心'),'link'=>$this->gen_url(array('app'=>'b2c', 'ctl'=>'wap_member', 'act'=>'index','full'=>1)));
        $this->path[] = array('title'=>app::get('b2c')->_('联系我们'),'link'=>'#');
        $GLOBALS['runtime']['path'] = $this->path;
        $this->pagedata['title'] = '联系我们';
        $this->page('wap/member/call_with.html');
    }
    ...
    //取消订单结果页面
    function cancel_result($order_id){
        //判断是否有取消订单记录
        $mdl_order_log = $this->app->model('order_log');
        $wheres = ["rel_id"=>$order_id,"behavior"=>'cancel',"bill_type"=>'order',"result"=>'SUCCESS'];
        $row = $mdl_order_log->getRow('*', $wheres);
        if (!empty($row)) {
            $row['status_name'] = '已取消';
        }
        // ee($row);
        ...
        $this->pagedata['title'] = '订单取消详情';
        $this->page('wap/member/cancel_result.html');
    }
    ...
    function doReceive(){
        ...
        if($sdf_order_member_id['member_id'] != $arrMember['member_id'])
        {
            $this->apiReturn(['error'=>1,'message'=>'请勿操作别人的收货']);
        }else{
            $arr_updates = array('order_id'=>$order_id,'received_status' =>'1','received_time'=>time());
            $mdl_order->save($arr_updates);
            $delivery_mdl = app::get('b2c')->model('order_delivery_time');
            ...
            if($orderLog->save($sdf_order_log)){
                $this->apiReturn(['error'=>0,'message'=>'已完成收货']);
            }else{
                $this->apiReturn(['error'=>1,'message'=>'收货失败']);
            }
        }
    }
    ...
    function afterlist($nPage=1){
        ...
        foreach($aData['data'] as $k => &$v) {
            ...
            foreach($v['goods_items'] as $k2 => &$v2) {
                ...
                //去除申请售后过得记录
                   $item_id=$k2;
                   $mdl_order = app::get('aftersales')->model('return_product');
                   $sdf_order = $mdl_order->getRow('*', array('item_id'=>$item_id));
                   if( $sdf_order){
                     $aftersales_data[$item_id]=$v['goods_items'][$k2];
                   $v['goods_items'][$k2]['is_apply']=2;
                     $aData['data'][$k]['return_id']=$sdf_order['return_id'];
                      $aData['data'][$k]['status']=$sdf_order['status'];
                          $aData['data'][$k]['received_time']=date('Y-m-d H:i:s',$sdf_order['add_time']);
                   }
        ...
        $this->pagedata['orders'] = $aData['data'];
        $this->pagedata['data'] = $aftersales_data;
        // ee( $aData['data']);
        $arr_args = array();
        $this->pagedata['title'] = '售后';
         //获取物流公司表数据
    ...
    function afterlist2($nPage=1){
       ...
       // ee($aData['data']);
        ...
        $this->pagedata['title'] = '售后';
        $this->pagination($nPage,$aData['pager']['total'],'afterlist',$arr_args);
        $this->page('wap/afterlist/refundlist(new).html');

    }
    public function add_aftersales($order_id,$item_id=0)
    {    
        // $this->path[] = array('title'=>app::get('b2c')->_('会员中心'),'link'=>$this->gen_url(array('app'=>'b2c', 'ctl'=>'wap_member', 'act'=>'index','full'=>1)));
        // $GLOBALS['runtime']['path'] = $this->path;
        $this->pagedata['title'] = '申请售后';
        $obj_return_policy = kernel::service("aftersales.return_policy");
        $arr_settings = array();
    ...
    public function return_save()
    {
        ...
        if ($obj_aftersales && $obj_aftersales->generate($aData, $msg))
        {
            ...
            $url= $this->gen_url(array('app'=>'b2c','ctl'=>'wap_member','act'=>'afterlist'));
    ...
    function afterrec_info($return_order_id){
        ...
        if (isset($aData['data']) && $aData['data'])
        {
            foreach($aData['data'] as $key=>$val){
                if($val['type']==1){
                  $aData['data'][$key]['type']="退货";
                }else{
                   $aData['data'][$key]['type']="换货";
                }
                $aData['data'][$key]['product_data'] = unserialize($val['product_data']);
                foreach($aData['data'][$key]['product_data'] as $gkey => $gval ){
                    $result = $products ->getRow('goods_id,spec_desc',array('bn'=>$gval['bn']));
        ...
        $tt = json_decode($this->pagedata['order']["customer_logistics"],true);
    ...
    //提交申请退款页面
    function refund_page($order_id){
        $this->pagedata['order_id'] = $order_id;
        $this->page('wap/member/order_refund_reason.html');
    }
    ...
    //退款结果页面
    function refund_result($order_id){
        $statusArr = [0=>'待处理',1=>'已拒绝',2=>'已退款'];
        //判断是否有退款申请记录
        $mdl_b2c_refund_apply = app::get('b2c')->model('refund_apply');
        $row = $mdl_b2c_refund_apply->getRow('*', ["order_id"=>$order_id,"disabled"=>'false']);
        if (!empty($row)) {
            $row['status_name'] = isset($statusArr[$row['status']]) ? $statusArr[$row['status']] : '';
        }
        // ee($row);
        $this->pagedata['row']=$row;
        $this->pagedata['order_id'] = $order_id;
        $this->pagedata['title'] = '退款详情';
        $this->page('wap/member/refund_result.html');
    }
    ...
    //实名认证($recert是否重新认证)
    function prove_page($recert=false){
        $mdl_merchant_type = $this->app->model('merchant_type');
        $isEdit = false;//是否允许编辑
        $mdl_prove= $this->app->model('prove');
        $row = $mdl_prove->getRow('*',['member_id'=>$this->app->member_id,'disabled'=>'false']);
        ...
        if (empty($row)) {
            ...
            //如果状态为['first','review','pass']则关闭编辑功能
            if ($recert && $row['status'] == 'nopass') {
                $isEdit = true;
            }
            ...
            foreach ($images as $image) {
                if (!empty($row[$image])) {
                    ...
                    $row[$image]['thumbImages'] =  $upload->showUploadedThumbFiles($row[$image], $image, $row['prove_id'],$isEdit);
                }
            }
        }
        // ee($row);
        $this->pagedata['isEdit'] = $isEdit;
        $this->pagedata['title'] = '实名认证';
        $this->pagedata['row'] = $row;
        $this->pagedata['merchant_type_rows'] = $merchant_type_rows;
        $this->page('wap/member/prove_page.html');
    }
    ...
    function do_prove(){
        ...
        $checkFields = [
            ...
            'region_id'        =>['is'=>1, 'name'=>'商户所在区域'],
        ];

        $imagesFields = [
            'store_img'        =>['is'=>0, 'name'=>'门店图片'],
            'license_img'      =>['is'=>0, 'name'=>'营业执照图片'],
            'id_back_img'      =>['is'=>0, 'name'=>'身份证背面图片'],
            'id_front_img'     =>['is'=>0, 'name'=>'身份证正面图片'],
        ];
        ...
        foreach ($checkFields as $key => $value) {
            if ($value['is'] === 1 && empty($datas[$key])) {
                $this->apiReturn(['error'=>1,'message'=> $value['name'] . '必须']);
            }
        ...
        if (empty($fields['prove_id'])) {
            unset($fields['prove_id']);
        }

        // if (!$mdl_prove->is_mobile($fields['inviter_mobile'])) {
        //     $this->apiReturn(['error'=>1,'message'=>'邀请人手机号格式错误!']);
        // }

        if (!$mdl_prove->is_mobile($fields['mobile'])) {
            $this->apiReturn(['error'=>1,'message'=>'手机号格式错误!']);
        }

        //用户地址合法判断
        $user_addr_num = $mdl_member_addrs->addrNum($member_id);
        if ($user_addr_num > 0) {
            //删除之前的收货地址
            $delAddrRes = $mdl_member_addrs->delete(['member_id'=>$member_id]);
            if (!$delAddrRes) {
                $db->rollback();
                $this->apiReturn(['error'=>1,'message'=>'清除之前的收货地址失败!']);
            }
        }
        ...
        $addr_res = $mdl_member_addrs->insert($addr_fields);
        if (!$addr_res) {
            $db->rollback();
            $this->apiReturn(['error'=>1,'message'=>'收货地址保存失败！']);
        }

        //更新members表的area、addr信息
        $mdl_members = app::get('b2c')->model('members');
        $members_res = $mdl_members->update(
            [
                'area'=>$datas['area'],
                'addr'=>$datas['addr']
            ],
        ...
        $upload = kernel::single('b2c_Think_upload');//文件上传类
        $files = $upload->thumbUploadedFiles();//缩略已经上传的图片

        foreach ($imagesFields as $image=>$imageRow) {
            //如果是添加，则每个图片都必须上传
            if (empty($datas['prove_id']) && empty($files[$image])) {
                $this->apiReturn(['error'=>1,'message'=> $imageRow['name'] . '必须']);
            }

            if (!empty($files[$image])) {
                $fields[$image] = $files[$image][0]['saveFile'];
            }
        }
        // ee($files);
        ...
        if (empty($datas['prove_id'])) {
            ...
            $res = $mdl_prove->insert($fields);
        }else{
            //到这里就变成重新认证了
            $fields['mtime'] = $ctime;
            $fields['status'] = 'first';//重新认证
            $res = $mdl_prove->update($fields,['prove_id'=>$datas['prove_id']]);
        }

        if (!$res) {
            $db->rollback();
            $this->apiReturn(['error'=>1,'message'=>'实名认证数据保存失败']);
        }

        $db->commit($transaction_status);
        $this->apiReturn(['error'=>0,'message'=>'操作成功']);
    }
    ...
    public function imagedel(){
        $wdatas = $_GET;
        $id = trim($wdatas['id']);
        $onlyFile = trim($wdatas['onlyFile']);//是否只删除图片
        $fullName = trim($wdatas['fullName']);//图片完整路径
        $type = trim($wdatas['type']);//文件存储子目录(门店|营业执照|身份证)[store_img|license_img|id_front_img|id_back_img]
        $upload = kernel::single('b2c_Think_upload');//文件上传类
        $fields = ['store_img','license_img','id_front_img','id_back_img'];//字段类型

        //如果单单删除图片,比如刚上传的还没有存数据库
        if ($onlyFile) {
            if (is_file($fullName) && file_exists($fullName)) {
                @unlink($fullName);
                $this->apiReturn();
            }
            $this->apiReturn(array('error' => 1, 'message' => '操作失败，请重试！!'));
        }

        if(!$id){
            $this->apiReturn(array('error' => 1, 'message' => 'id值必须填写！'));
        }

        if (!in_array($type, $fields)) {
            $this->apiReturn(array('error' => 1, 'message' => '子目录不存在！'));
        }

        $wheres = array();
        $wheres['status'] = '1';
        $wheres['sn'] = array('in',"{$sn}");

        //id值是否有效
        $mdl_prove= $this->app->model('prove');
        $wheres = ['prove_id'=>$id,'member_id'=>$this->app->member_id,'disabled'=>'false'];
        $row = $mdl_prove->getRow('*', $wheres);
        // ee($this->app->member_id);
        if(empty($row)){
            $this->apiReturn(array('error' => 1, 'message' => '无效的id值'));
        }

        //删除数据表图片记录和图片文件
        if(!empty($row[$type])){
            $res = $mdl_prove->update([$type=>''], $wheres);
        }

        $images = $upload->getImageSizes($row[$type],$type);
        //删除大中小原图
        foreach ($images as $image) {
      ...
      //保证金
    public function bond_page(){
                 $this->pagedata['title']='保证金';
                 //按照商品id定义写死921的单价
                 $goods_id=952;
                ...
                //判断该会员交没有交保证金
                   ...
                   $member_id = $this->app->member_id;
                  $mdl_order= app::get('b2c')->model('orders');
                 $result= $mdl_order->getRow('*', array('pay_status'=>1,'member_id'=>$member_id,'promotion_type'=>'bail'));
                //保证金交没交
                if($result){
                        $this->pagedata['is_pay'] = 1;
                        $this->pagedata['result'] = $result;
                }

                //有没有退款申请
                  $refund_apply= $mdl_order->getRow('*', array('pay_status'=>6,'member_id'=>$member_id,'promotion_type'=>'bail'));
                if($refund_apply){
                        $this->pagedata['is_refund'] = 1;
                        $this->pagedata['result'] = $refund_apply;
                       
                }
 

                 $goods = $mdl_goods->getRow('price', array('goods_id'=>$goods_id));
                 $this->pagedata['goods'] = $goods;
                 $this->pagedata['payment_app_status'] = 1;
                 $this->pagedata['payment'] = 2;
                  // ee($this->pagedata);
                 $this->page('wap/member/bond_page.html');  
    }
