...
class b2c_ctl_wap_tools extends b2c_frontpage{
    ...
    public function upfile(){
        ...
        $posts = $_POST;
        $folder = $posts['folder'];//上传文件夹
        //数据库保存的图片类型,如果保存在images_goods里面则type可能值为plate、block、discount;
        //如果保存在images里面则为1004060000类似编号
        $savetype = $posts['savetype'];
        if ($upload->_is_uploaded_file('file')) {
            $uploads = $upload->_uploadsImages(array('files' => 'file' ,'folder' => $folder));
            $datas = $uploads['info'][0];
            $returnData = array(
                'folder'   => $folder,
                'savetype'     => $savetype,
                'saveFile' => $datas['saveFile'],
                'fullName' => $datas['fullName'],
            );
            $this->apiReturn(['data' => $returnData]);
        }
        $this->apiReturn(['error' => 1, 'message' => "文件上传失败！请重试"]);
    }
