...
class b2c_ctl_wap_paycenter extends wap_frontpage{
    ...
    public function dopayment($pay_object='order',$combination_pay = false)
    {
        ...
        if($_POST['order_type']=='bail'){
            $bail=$_POST;
            $tt = json_decode($bail['payment'],true);
            $_POST= Array
            (
                payment=> Array
                    (
                        order_id =>$bail['order_id'],
                        combination_pay =>'false',
                        def_pay =>Array
                            (
                                cur_money => $bail['order_money'],
                                pay_app_id =>$tt['pay_app_id'],
                            ),
        ...
        if($_POST['payment']['def_pay']['pay_app_id']=='cpcnquick'){
            if(!$_POST['TxSNBinding']){
                echo '<script>alert("未选择银行卡！");window.history.go(-1);</script>';exit;
            }
        }
        ...
        if ($pay_object)
        {
            ...
            switch ($sdf['pay_object'])
            {
                case 'order':
                //货到付款会员不用登陆校验
                if($_POST['payment']['payment_offline']!="-1"){
