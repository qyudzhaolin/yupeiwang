...
class b2c_ctl_wap_cart extends wap_frontpage{
    ...
    public function index(){
        ...
        $this->pagedata['pageLocation'] = 'cart';

        //获取原产地
        // if(!empty($this->pagedata['aCart']['object']['goods'])){
        //     $mdl_gprovenance = $this->app->model('gprovenance');
        //     foreach ($this->pagedata['aCart']['object']['goods'] as $key => $value) {
        //         $areaRow = $mdl_gprovenance->getAreaByGoodsId($value['obj_items']['products'][0]['goods_id']);
        //         $area = $areaRow ? $areaRow['area'] : '';
        //         $this->pagedata['aCart']['object']['goods'][$key]['obj_items']['products'][0]['area']=$area;
        //     }

        // }
        // ee($this->pagedata['aCart']['object']['goods']);
         $site_order_quota = app::get('b2c')->getConf('site.order_quota'); //是否开启自动取消未支付订单
      
        if( $site_order_quota=='true'){
            $site_order_quota_money = app::get('b2c')->getConf('site.order_quota_money');
            $this->pagedata['order_quota']=$site_order_quota; 
            $this->pagedata['site_order_quota']='true'; 
            $this->pagedata['quota_money']=$this->objMath->formatNumber($site_order_quota_money,2);
        }
    ...
    public function checkout_result($isfastbuy=0){
       ...
       /* $objproducts = $this->app->model('products');
        ...
        if($prepare)
        {
          ...
          $this->pagedata['total_amount']=$this->objMath->formatNumber($total_amount,2);
    ...
    */
    ...
    function invoice(){
    	 $this->page('wap/cart/checkout/invoice.html');
    }
    function index2(){
         $this->page('wap/cart/checkout/index2.html');
    }
}
