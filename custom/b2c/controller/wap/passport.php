...
class b2c_ctl_wap_passport extends wap_frontpage{
    ...
    public function zhuce(){
        ...
        $this->page('wap/passport/zhuce.html');
    }
   ...
   /**
    ...
    public function create(){
        ...
        if($_POST['pam_account']['user_name']!=null){
           $filter = array(
             'login_account'=>$_POST['pam_account']['user_name'],
             'login_type'=>'local',
            );
             $row = app::get('pam')->model('members')->getList('login_account', $filter);
             if($row){
                $msg = app::get('b2c')->_('该用户名被占用!');
                $this->splash('failed',null,$msg,'','',true);exit;
            }
        ...
        //验证密码
         if($_POST['pam_account']['login_password']!=null){
           
             if($_POST['pam_account']['login_password']!=$_POST['pam_account']['login_password2']){
                $msg = app::get('b2c')->_('两次输入密码不一致!');
                $this->splash('failed',null,$msg,'','',true);exit;
            }
        ...
        if($_POST['pam_account']['user_name']){
           $saveData['pam_account']['user_name'] = $_POST['pam_account']['user_name'];
        }
    ...
    public function send_vcode_sms(){
        ...
        if($type == 'forgot'){
          //校验手机号是否注册过
            ...
            $members_mobile = $members->getRow('member_id',array('login_account'=>$mobile));
            if(!$members_mobile){
            $msg = app::get('b2c')->_('该手机号没有注册过无法找回密码');
            $this->splash('failed',null,$msg,'','',true);exit();
            }
        }
        //这里是验证码
   ...
   public function resetpasswordnew(){
        ...
        $this->check_login();
         if($_POST['login_password']!=$_POST['psw_confirm']){
                $msg = app::get('b2c')->_('两次输入密码不一致!');
                $this->splash('failed',null,$msg,'','',true);exit;
            }
        $_POST['send_type'] ='mobile';
        ...
        $userVcode = kernel::single('b2c_user_vcode');
        if( !$vcodeData = $userVcode->verify($_POST['mobilevcode'],$_POST['send_type'],'forgot')){
            $msg = app::get('b2c')->_('验证码错误');
            $this->splash('failed',null,$msg,'','',true);exit;
        }
        $_POST['key'] = $userVcode->get_vcode_key($_POST['send_type'],'forgot');
