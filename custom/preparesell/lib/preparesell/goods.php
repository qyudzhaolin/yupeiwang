<?php
class preparesell_preparesell_goods extends b2c_goods_check_abstract{

    #商品列表页、商品详情页参加preparesell活动的货品 价格、标签
    function check_preparesell_goods_list(&$products){
        $preparesell_goods_mdl = app::get('preparesell')->model('preparesell_goods');
        $filter = array(
            'status'=>'true',
        ...
        foreach($products as $key=>$product){
            ...
            $list = $preparesell_goods_mdl->getRow('promotion_price,type_id',$filter);
            if($list){
                ...
                $products[$key]['app_name']="preparesell";
            }
        }
    }
    ...
    function _getTypeName($id){
        $type_mdl = app::get('preparesell')->model('promotions_type');
    ...
    public function check_product_delete($product_id,&$msg){
        ...
        $list = $preparesell_goods_mdl->getRow("status,type_id",$filter);
        if($list){
