...
class preparesell_preparesell_order{
    ...
    function cancel($order_id,&$msg){
        ...
        foreach($order_items as $value){
            $gtimeout = $this->_get_preparesell_goods_timeout($value['product_id'],$value['nums']);
    ...
    function _get_preparesell_goods_timeout($product_id,$nums){
        ...
        $result = $mdl_preparesell_goods->getRow("timeout",$filter);
        if($result){
            $mdl_preparesell_goods->db->exec('UPDATE sdb_preparesell_preparesell_goods SET initial_num = initial_num+'.$nums.' where product_id='.$product_id);
    ...
    function check_order($orders){
        $cancel_mdl = app::get('preparesell')->model('cancelorder');
