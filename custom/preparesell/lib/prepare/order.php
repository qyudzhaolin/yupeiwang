...
class preparesell_prepare_order{
    ...
    function sava_prepare_order($data){
        ...
        $order_fund_id = $prepareOrderModel->saveOrderFund($prepare_id['prepare_id'],$data['order_id'],$data['product_id']);
        // ee(sql(),0);
    ...
    function get_prepare_info($data)
    {
        ...
        foreach ($prepare_order as $key => $value) {
            ...
            $prepare[$value['order_id']]['prepare_status']=$value['prepare_status'];
        }
        return $prepare;
