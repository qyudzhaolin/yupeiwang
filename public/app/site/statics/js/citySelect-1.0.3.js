...
(function ($, window) {
            ...
            if (citydata < 1) {
                ...
                if (letter === key) {
                    ...
                    $.each(value, function (lkey, lvalue) {
                        ...
                        html += '<a href="javascript:;" class="caller" data-pid="' + lvalue.pid + '" data-id="' + lvalue.id + '" data-title="' + name + '" data-letter="' + lvalue.letter + '">' + name + '</a>';
                ...
                html += '<li class="caller" data-pid="' + value.pid + '" data-id="' + value.id + '" data-title="' + name + '" data-letter="' + value.letter + '">' +
    ...
    functionality.singleAchieve = function (event) {
        ...
        //存储选中的值
        this.values = [];
        this.values.push({ 'name': name, 'id': id, 'pid': pid });
        ...
        this.$selector.find('.city-input').before('<span data-id="'+ id +'" data-pid="'+ pid +'">'+ name +'<i class="del"></i></span>').find('.input-search').val('');
		...
		//if($('#alertBox-shade').length) $('#alertBox-shade').remove();
    ...
    functionality.multiAchieve = function (event) {
        ...
        if (!hasActive) {
            ...
            if ($.inArray(name, self.multiSelectResult) < 0) {
                ...
                self.provinceId.push(pid);

                //拼接生成选中值
                joinSpan = '<span data-id="'+ id +'" data-pid="'+ pid +'">'+ name +'<i class="del"></i></span>';
            ...
            self.provinceId.splice($.inArray(pid, self.provinceId), 1);
        ...
        //存储选中的值
        self.values = [];
        self.multiSelectResult.length > 0 ? self.values.push({ 'name': self.multiSelectResult, 'id': self.multiSelectResultId, 'pid': self.provinceId }) : '';
            ...
            for (var i = 0; i < cityArray.length; i++) {
                ...
                $.each(self.newCityData, function (key, value) {
                    ...
                    if (cityArray[i] === value.name) {
                        ...
                        self.provinceId.push(value.pid);

                        //添加选中状态
                        self.$selector.find('.caller[data-id="'+ value.id +'"]').addClass('active');

                        //选中的值拼接dom
                        joinSpan = '<span data-id="'+ value.id +'" data-pid="'+ value.pid +'">'+ name +'<i class="del"></i></span>';
    ...
    $(document).on('click.cityselect', function (event) {
        ...
        //新增功能：不点击区域不允许关闭//如果有遮罩层说明还未设置配送区域
        if ($citySelect.find(event.target).length < 1 && !$('#alertBox-shade').length) {
			...
			//TODO[点击logo区域以外都隐藏]
			if ($('.areaBox').length && $('.areaBox').find(event.target).length < 1) {
                $citySelect.addClass('hide').removeClass('down');
				//if($('#alertBox-shade').length) $('#alertBox-shade').remove();
			}
        }
    });
