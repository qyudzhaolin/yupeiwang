window.LArea = (function() {
    ...
    MobileArea.prototype = {
        ...
        bindEvent: function() {
            ...
            function rollGear(target) {
                ...
                target["int_" + target.id] = setInterval(function() {
                    ...
                    if (stopGear) {
                        var gearVal = Math.abs(pos) / 2 || 0;//#TODO#修复呼出时候点击报错bug
        ...
        finish: function(e) {
			...
			var locationText = provinceText + ((cityText)?(',' + cityText):(''))+ ((countyText)?(',' + countyText):(''));
            _self.value = [provinceVal, cityVal, countyVal];
            if(this.valueTo){
                this.valueTo.value= provinceCode +((cityCode)?(',' + cityCode):('')) + ((countyCode)?(',' + countyCode):(''));
            }
			
			//设置this的locationText、locationValue
			if(countyText){
				_self.locationText = countyText;
			}else if(cityText){
				_self.locationText = cityText;
			}else if(provinceText){
				_self.locationText = provinceText;
			}
			
			if(countyCode){
				_self.locationValue = countyCode;
			}else if(cityCode){
				_self.locationValue = cityCode;
			}else if(provinceCode){
				_self.locationValue = provinceCode;
			}
						
			if(_self.params.callback){
				_self.params.callback(_self);
			}
            _self.close(e);
        },
        close: function(e) {
