...
function theme_widget_right_news(&$setting,&$smarty){
...
function article_new_foo($lv=1, $iNodeId=1, $limit, $showallart, $oAN, $oMAI, &$tmp, $setting) {
    ...
    if( $showallart ) {
        ...
        $tmp['article'] = $oMAI->getList_1('*', array('node_id'=>$iNodeId, 'platform'=>'pc', 'ifpub'=>'true', 'pubtime|lthan'=>time(),'nochildren'=>true),0, $limit,"{$setting['order_type']} {$setting['order']} ");
    }
}
...
function article_new_show(&$smarty, $tmp, $setting, &$html, $lv=0, &$limit) {
    if($setting['shownode'] && $lv!=0) {
        ...
        $html[] = article_new_html($lv, $url, $tmp['info']['node_name'],$tmp['info']['node_name'],$typeClass);
    }
    ...
    if($tmp['article']) {
        ...
        foreach ($tmp['article'] as $idx=>$row) {
            ...
            if($limit>0){
                if($idx == 0){
                    $html[$key] = "<li class='article-index-list'><ul>".article_new_html($tmp_lv, $url, $article_title,$attribute_title,'article-index');
                }elseif($idx == ($len-1)){
                    $html[$key] = article_new_html($tmp_lv, $url, $article_title,$attribute_title,'article-index')."</ul></li>";
                }else{
                    $html[$key] = article_new_html($tmp_lv, $url, $article_title,$attribute_title,'article-index');
                }
            }
...
function article_new_html($lv, $url, $name,$attribute_title,$type) {
    return <<<EOF
<li class="{$type} lv-{$lv} news-item">
    <a href="{$url}" title="{$attribute_title}">{$name}</a>
</li>
EOF;
}
?>
